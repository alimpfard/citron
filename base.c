...
ctr_object *ctr_object_message(ctr_object * myself, ctr_argument * argumentList)
{
	...
	ctr_size length =
	    fmin(arr->value.avalue->tail - arr->value.avalue->head,
		 arr->value.avalue->length);
	ctr_size i = 0;
	ctr_argument *args = ctr_heap_allocate(sizeof(ctr_argument));
	ctr_argument *cur = args;
	for (i = arr->value.avalue->tail; i < length; i++) {
		if (i > 0) {
			cur->next = ctr_heap_allocate(sizeof(ctr_argument));
			cur = cur->next;
		}
		cur->object = arr->value.avalue->elements[i];
	}
...
ctr_object *ctr_object_respond(ctr_object * myself, ctr_argument * argumentList)
{
	...
	ctr_object *arr;
	ctr_object *answer;
	arr = ctr_internal_create_standalone_object(CTR_OBJECT_TYPE_OTARRAY);
	arr->link = CtrStdArray;
	ctr_array_push(arr, argumentList);
	argumentList->object = ctr_array_to_string(arr, NULL);
	ctr_internal_delete_standalone_object(arr);
	answer = ctr_object_send2remote(myself, argumentList);
	return answer;
}
...
{
	return
	    ctr_build_bool(ctr_internal_cast2bool(argumentList->object)->value.
				...
				ctr_argument * argumentList)
{
	...
	return argumentList->object->info.type ==
	    CTR_OBJECT_TYPE_OTBLOCK ? ctr_block_run(argumentList->object, NULL,
						    NULL) : argumentList->
	    object;
}
...
ctr_object *ctr_bool_either_or(ctr_object * myself, ctr_argument * argumentList)
{
	if (myself->value.bvalue) {
		if (argumentList->object->info.type == CTR_OBJECT_TYPE_OTBLOCK)
			return ctr_block_run(argumentList->object, NULL, NULL);
		return argumentList->object;
	} else {
		if (argumentList->next->object->info.type ==
		    CTR_OBJECT_TYPE_OTBLOCK)
			return ctr_block_run(argumentList->next->object, NULL,
					     NULL);
...
ctr_object *ctr_number_times(ctr_object * myself, ctr_argument * argumentList)
{
	...
	for (i = 0; i < t; i++) {
		CTR_INTER(iname);
			...
			ctr_object* result = ctr_block_run_here(block, arguments, NULL);
			if(result != block)
				ctr_internal_delete_standalone_object(result);
			if (CtrStdFlow == CtrStdContinue)
				CtrStdFlow = NULL;	/* consume continue */
			if (CtrStdFlow)
				break;
		CTR_INTER_CLOSE(iname, indexNumber);
	}
	ctr_close_context();
	ctr_heap_free(arguments);
	if (CtrStdFlow == CtrStdBreak)
				  ...
				  ctr_argument * argumentList)
{
	...
	while (((forward && curValue <= endValue)
		|| (!forward && curValue >= endValue)) && !CtrStdFlow) {
			...
			ctr_object* result = ctr_block_run_here(codeBlock, arguments, NULL);
			if(result != codeBlock)
				ctr_internal_delete_standalone_object(result);
			if (CtrStdFlow == CtrStdContinue)
				CtrStdFlow = NULL;	/* consume continue and go on */
			curValue += incValue;
		CTR_INTER_CLOSE(iname, arg);
	}
	ctr_heap_free(arguments);
	ctr_close_context();
	if (CtrStdFlow == CtrStdBreak)
 ...
 * [Number] bitOr: [Number]
 *
 * Bitwise or (|) operation on two (int) numbers
 */
 ...
 *
 * Bitwise and (&) operation on two (int) numbers
 */
 ...
 *
 * Bitwise xor (^) operation on two (int) numbers
 */
 ...
 *
 * returns whether object is constructible from another
 */
...
{
	return ctr_build_number_from_float((ctr_number) myself->value.svalue->
					   vlen);
}
				...
				default:
					args->object =
					    ctr_send_message(ctr_array_get
							     ...
							     NULL);
					char *strr =
					    ctr_heap_allocate_cstring(args->
								      object);
...
ctr_object *ctr_string_slice(ctr_object * myself, ctr_argument * argumentList)
{
	...
	long a, b;
	a = getBytesUtf8(myself->value.svalue->value, 0,
			 argumentList->object->value.nvalue);
	str_cut(myself->value.svalue->value, myself->value.svalue->vlen,
		a, (b =
		    getBytesUtf8
		    (myself->value.svalue->value,
		     a, argumentList->next->object->value.nvalue)));
			       ...
			       int left)
{
	...
	padding =
	    (uint16_t) ctr_internal_cast2number(argumentList->object)->value.
	    nvalue;
	a->object =
				   ...
				   ctr_argument * argumentList)
{
	...
	char byte;
	byte =
	    (uint8_t) ctr_internal_cast2number(argumentList->object)->value.
	    nvalue;
				       ...
				       ctr_argument * argumentList)
{
	...
	len =
	    (size_t) ctr_internal_cast2number(argumentList->object)->value.
	    nvalue;
	if (len == 0)
		return ctr_build_empty_string();
...
inline void ctr_capture_refs_(ctr_tnode * ti, ctr_object * block, int noerror)
{
	ctr_tlistitem *li;
	ctr_tnode *t;
	// if (indent>20) exit(1);
 ...
 * get property
 */
ctr_object *ctr_object_get_property(ctr_object * myself,
				    ctr_argument * argumentList)
{
	ctr_object *id = argumentList->object;
	ctr_object *hash = ctr_send_message(id, "iHash", 5, NULL);
	ctr_object *prop =
	    ctr_internal_object_find_property_with_hash(myself, id,
							hash->value.nvalue, 0);
	return prop ? prop : CtrStdNil;
}
