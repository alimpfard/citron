	   ...
	   (defs->info.type == CTR_OBJECT_TYPE_OTARRAY && i < defs->value.avalue->head - defs->value.avalue->tail); i++)
	{
	  switch (defs->info.type)
	    ...
	    case CTR_OBJECT_TYPE_OTARRAY:
	      ctr_assign_value_to_local (myself->value.avalue->elements[i], defs->value.avalue->elements[i]);
	      break;
	    default:
...
ctr_object *
ctr_reown_obj (ctr_object * myself, ctr_argument * argumentList)
{
...
static char const *
ctr_ast_modifier_str (char modifier)
{
  switch (modifier)
    {
    case 1:
...
static char
ctr_ast_modifier_fstr (char *mod)
{
...
char **
str_split (char *a_str, const char a_delim, int *count_out)
{
  char **result = 0;
  size_t count = 0;
  char *tmp = a_str;
...
void *
ctr_coroutine_close (struct schedule *S)
{
  coroutine_close (S);
  return NULL;
}
...
void
ctr_coroutine_run_block (struct schedule *S, void *ud)
{
 ...
 */
ctr_object *
ctr_coro_make (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_coro_new (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_coro_error (ctr_object * myself, ctr_argument * argList)
{
 ...
 */
ctr_object *
ctr_coro_resume (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_coro_yield (ctr_object * myself, ctr_argument * argumentList)
{
  struct ctr_coro_coroutine *sco = myself->value.rvalue->ptr;
  int co = sco->co;
  struct schedule *S = sco->scheduler->value.rvalue->ptr;
...
ctr_object *
ctr_coro_state (ctr_object * myself, ctr_argument * argumentList)
{
  struct ctr_coro_coroutine *sco = myself->value.rvalue->ptr;
  int co = sco->co;
  struct schedule *S = sco->scheduler->value.rvalue->ptr;
...
ctr_object *
ctr_coro_isrunning (ctr_object * myself, ctr_argument * argumentList)
{
  struct ctr_coro_coroutine *sco = myself->value.rvalue->ptr;
  int co = sco->co;
  struct schedule *S = sco->scheduler->value.rvalue->ptr;
...
ctr_object *
ctr_string_to_symbol (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_object_inherit (ctr_object * myself, ctr_argument * argumentList)
{
...
initiailize_base_extensions ()
{
  ...
  ctr_internal_create_func (CtrStdSymbol, ctr_build_string_from_cstring ("iHash"), &ctr_symbol_ihash);
...
int
mod_accessible (char const *mname)
{
