...
ctr_object *ctr_array_new(ctr_object *myclass, ctr_argument *argumentList) {
  ...
  if (argumentList && argumentList->object &&
      argumentList->object->info.type == CTR_OBJECT_TYPE_OTNUMBER) {
...
ctr_object *ctr_array_all(ctr_object *myself, ctr_argument *argumentList) {
  ...
  if ((fn = argumentList->object)) {
    ...
    for (i = myself->value.avalue->tail; i < myself->value.avalue->head; i++) {
      ...
      if ((el->info.type == CTR_OBJECT_TYPE_OTBOOL && el->value.bvalue) ||
...
ctr_object *ctr_array_fill(ctr_object *myself, ctr_argument *argumentList) {
  ...
  if (memb->info.type == CTR_OBJECT_TYPE_OTBLOCK ||
      memb->info.type == CTR_OBJECT_TYPE_OTNATFUNC) {
    ctr_object *_i =
...
uint32_t ctr_internal_string_hash(ctr_object *key) {
  ...
  if (unlikely(key->info.type != CTR_OBJECT_TYPE_OTSTRING)) {
    CtrStdFlow =
        ctr_build_string_from_cstring("Only Strings allowed as HashMap keys");
    return 0xdead;
  }
...
ctr_object *ctr_hmap_fmap(ctr_object *myself, ctr_argument *argumentList) {
  ...
  for (khiter_t k = kh_begin(h); k != kh_end(map); ++k)
    if (kh_exist(map, k)) {
      ctr_object *val =
          ctr_block_run(blk, &(ctr_argument){kh_key(map, k), NULL}, NULL);
      if (CtrStdFlow) {
        if (CtrStdFlow == CtrStdContinue) {
          CtrStdFlow = NULL;
          continue;
...
ctr_object *ctr_hmap_contains(ctr_object *myself, ctr_argument *argumentList) {
  ...
  if (!map)
    ctr_hmap_throw_error();
  return ctr_build_bool(kh_get(ctr_hmap_t, map, argumentList->object) !=
                        kh_end(map));
}
