...
ctr_object *ctr_array_new(ctr_object * myclass, ctr_argument * argumentList)
{
	...
	s->value.avalue->elements =
	    (ctr_object **) ctr_heap_allocate(sizeof(ctr_object *) * 2);
...
ctr_object *ctr_array_push(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if (myself->value.avalue->length <= (myself->value.avalue->head + 1)) {
		...
		myself->value.avalue->elements =
		    (ctr_object **) ctr_heap_reallocate(myself->value.
							avalue->elements,
							(sizeof(ctr_object *) *
							 (myself->value.avalue->
							  length)));
	...
	*(myself->value.avalue->elements + myself->value.avalue->head) =
	    pushValue;
...
ctr_object *ctr_array_multiply(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if (mand->info.type == CTR_OBJECT_TYPE_OTARRAY) {
		...
		for (ctr_size i = myself->value.avalue->tail;
			...
			ctr_object *elem0 =
			    *(myself->value.avalue->elements + i);
			for (ctr_size j = mand->value.avalue->tail;
			     j < mand->value.avalue->head; j++) {
				ctr_object *lst =
				    ctr_array_new(CtrStdArray, NULL);
				args->object = elem0;
				ctr_array_push(lst, args);
				args->object =
				    *(mand->value.avalue->elements + j);
				ctr_array_push(lst, args);
...
ctr_object *ctr_array_map(ctr_object * myself, ctr_argument * argumentList)
{
	...
	for (i = myself->value.avalue->tail; i < myself->value.avalue->head;
		...
		idx->value.nvalue = (ctr_number) i;
		arguments->object = idx;
...
ctr_object *ctr_array_map_v(ctr_object * myself, ctr_argument * argumentList)
{
	...
	for (i = myself->value.avalue->tail; i < myself->value.avalue->head;
	     i++) {
		pushArg->object = (myself->value.avalue->elements[i]);
		ctr_block_run_here(func, pushArg, func);
		if (CtrStdFlow == CtrStdContinue) {
			CtrStdFlow = NULL;
			continue;
		}
				    ...
				    ctr_argument * argumentList)
{
	...
	ctr_open_context();
	for (ctr_size s =
	     ceil((myself->value.avalue->head -
		   myself->value.avalue->tail) / (double)icount), i = 0; i < s;
	     i++) {
		ctr_object *idx =
		    ctr_internal_create_standalone_object
		    (CTR_OBJECT_TYPE_OTNUMBER);
		idx->value.nvalue = (ctr_number) (i * icount);
		ctr_object *idx2 =
		    ctr_internal_create_standalone_object
		    (CTR_OBJECT_TYPE_OTNUMBER);
		idx2->value.nvalue = (ctr_number) (icount);
		args->object = idx;
		args->next = caa;
		args->next->object = idx2;
		parr = ctr_array_slice(myself, args);
		while (parr->value.avalue->head - parr->value.avalue->tail <
		       icount) {
			args->object = fill;
		...
		args->object = parr;
		args->next = NULL;
		ctr_block_run_here(func, args, parr);
		ctr_internal_delete_standalone_object(idx);
		ctr_internal_delete_standalone_object(idx2);
	}
	ctr_close_context();
	ctr_heap_free(args);
	ctr_heap_free(caa);
...
ctr_object *ctr_array_unshift(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if (myself->value.avalue->tail > 0) {
		...
		if (myself->value.avalue->length <=
		    (myself->value.avalue->head + 1)) {
			myself->value.avalue->length =
			    myself->value.avalue->length * 3;
			myself->value.avalue->elements =
			    (ctr_object **) ctr_heap_reallocate(myself->
								value.avalue->
								elements,
								(sizeof
								 (ctr_object *)
		...
		myself->value.avalue->head++;
		memmove(myself->value.avalue->elements + 1,
			myself->value.avalue->elements,
			myself->value.avalue->head * sizeof(ctr_object *));
	}
	*(myself->value.avalue->elements + myself->value.avalue->tail) =
	    pushValue;
	return myself;
}
...
ctr_object *ctr_array_reverse(ctr_object * myself, ctr_argument * argumentList)
{
	...
	ctr_size i = 0;
	ctr_argument *args = ctr_heap_allocate(sizeof(ctr_argument));
	for (i = myself->value.avalue->head; i > myself->value.avalue->tail;
	     i--) {
		args->object = myself->value.avalue->elements[i];
		ctr_array_push(newArr, args);
	}
	ctr_heap_free(args);
	return newArr;
...
ctr_object *ctr_array_join(ctr_object * myself, ctr_argument * argumentList)
{
	...
	for (i = myself->value.avalue->tail; i < myself->value.avalue->head;
		...
		if (len == 0) {
			...
			result =
			    ctr_heap_reallocate(result, sizeof(char) * len);
			memcpy(result + pos, glue->value.svalue->value, glen);
			pos += glen;
		}
		memcpy(result + pos, str->value.svalue->value,
		       str->value.svalue->vlen);
	}
...
ctr_object *ctr_array_get(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if (myself->value.avalue->head <= (i + myself->value.avalue->tail)
	    || i < 0) {
		CtrStdFlow =
...
{
	ctr_number d = 0;
	d = (ctr_number) myself->value.avalue->head -
	    myself->value.avalue->tail;
	return
	    ctr_build_number_from_float(fmin(d, myself->value.avalue->length));
}
...
ctr_object *ctr_array_zip(ctr_object * myself, ctr_argument * argumentList)
{
	...
	struct arr_element_t *elem_indices =
	    ctr_heap_allocate(sizeof(struct arr_element_t) *
			      (myself->value.avalue->head -
			       myself->value.avalue->tail));
	ctr_size all_iter = 1;
	ctr_size all_ind =
	...
	for (ctr_size i = 0;
		...
		struct arr_element_t arredes = {
			.elem =
			    *(myself->value.avalue->elements +
			      myself->value.avalue->tail + i),
			.state = 0,
		};
		arredes.max =
		    arredes.elem->info.type ==
		    CTR_OBJECT_TYPE_OTARRAY ? (int)(arredes.elem->value.
						    avalue->head -
						    arredes.elem->value.avalue->
						    tail) : -1;
	...
	for (ctr_size lit = 0; lit < all_iter; lit++) {
		...
		for (ctr_size i = 0;
			...
			if (arrdes.max > -1) {
				elem =
				    arrdes.elem->value.avalue->
				    elements[arrdes.elem->value.avalue->tail +
					     arrdes.state];
				if (!did_shit) {
					arrdes.state++;
					did_shit = 1;
...
ctr_object *ctr_array_zip_with(ctr_object * myself, ctr_argument * argumentList)
{
	...
	for (ctr_size i = 0;
		...
		arredes.max =
		    arredes.elem->info.type ==
		    CTR_OBJECT_TYPE_OTARRAY ? arredes.elem->value.avalue->head -
		    arredes.elem->value.avalue->tail : -1;
		all_iter *= (arredes.max > -1 ? arredes.max : 1);
		elem_indices[i] = arredes;
	}
...
ctr_object *ctr_array_add(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if (otherArray->info.type == CTR_OBJECT_TYPE_OTARRAY) {
		for (i = otherArray->value.avalue->tail;
		     i < otherArray->value.avalue->head; i++) {
			ctr_object *elnum =
					 ...
					 ctr_argument * provided)
{
	if (!arr)
		return NULL;
	ctr_size i = arr->value.avalue->tail, arr_max_len =
	    arr->value.avalue->head - arr->value.avalue->tail;
	if (!provided)
				     ...
				     ctr_argument * argumentList)
{
	ctr_object *selector = argumentList->object,
	    ...
	    *ifexp = argumentList->next->next
	    && argumentList->next->next->object ? argumentList->next->next->
	    object : NULL, *result =
	    ctr_array_new(CtrStdArray, NULL), *elem, *old_from;
 ...
 restart:;
	...
	if (from->info.type == CTR_OBJECT_TYPE_OTARRAY) {
		printf("%p\n", from);
		for (ctr_size i = from->value.avalue->tail;
		     i < from->value.avalue->head; i++) {
			elem = from->value.avalue->elements[i];
			if (elem->info.type == CTR_OBJECT_TYPE_OTARRAY)
				argument =
				    ctr_array_to_argument_list(elem, argument);
			else {
			...
			if (ifexp) {
				ctr_object *clause =
				    ctr_internal_cast2bool(ctr_block_run
							   (ifexp, argument,
							    ifexp));
			...
			ctr_object *maybe =
			    ctr_block_run(selector, argument, selector);
...
ctr_object *ctr_array_assign(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if (to->info.type == CTR_OBJECT_TYPE_OTARRAY) {
		int other;
		int saw_catch_all = 0;
		for (other = (i = to->value.avalue->tail);
		     i < to->value.avalue->head; other++, i++) {
			ctr_object *elnum =
			    ctr_build_number_from_float((ctr_number) i);
			elnumArg->object = elnum;
			ctr_object *to_elem;
			...
			if (accArg->object->info.type ==
			    ...
			    (ctr_build_string_from_cstring("*"),
			     ctr_send_message_variadic(accArg->object, "at:", 3,
						       1,
						       ctr_build_number_from_float
						       (0)))) {
				...
				if (saw_catch_all) {
					ctr_object *cc =
					    ctr_build_string_from_cstring
					    ("Pattern (");
					ctr_send_message_variadic(cc, "append:",
								  7, 1, to);
					ctr_send_message_variadic(cc, "append:",
								  7, 1,
								  ctr_build_string_from_cstring
				...
				int skip = ctr_array_count(myself,
							   NULL)->value.nvalue -
				    to->value.avalue->head - i;
				to_elem =
				    ctr_send_message_variadic(myself,
							      "from:length:",
							      12, 2,
							      ctr_build_number_from_float
							      (other -
				...
				ctr_argument * argumentList)
{
	...
	if (myself->value.avalue->tail == myself->value.avalue->head) {
		newArgumentList->object =
		    ctr_build_string_from_cstring(myself->value.
						  avalue->immutable ? "[" :
						  CTR_DICT_CODEGEN_ARRAY_NEW);
		string = ctr_string_append(string, newArgumentList);
	} else {
		newArgumentList->object =
		    ctr_build_string_from_cstring(myself->value.
						  avalue->immutable ? "[" :
						  CTR_DICT_CODEGEN_ARRAY_NEW_PUSH);
		string = ctr_string_append(string, newArgumentList);
	}
	for (i = myself->value.avalue->tail; i < myself->value.avalue->head;
	     i++) {
		...
		if (!arrayElement) {
			newArgumentList->object =
			    ctr_build_string_from_cstring("':NULLPtr:'");
			string = ctr_string_append(string, newArgumentList);
			   ...
			   || arrayElement->info.type == CTR_OBJECT_TYPE_OTNIL)
		{
			...
			string = ctr_string_append(string, newArgumentList);
			newArgumentList->object =
			    ctr_string_quotes_escape(arrayElement,
						     newArgumentList);
			string = ctr_string_append(string, newArgumentList);
			newArgumentList->object =
			    ctr_build_string_from_cstring("'");
		...
		if ((i + 1) < myself->value.avalue->head) {
			newArgumentList->object =
			    ctr_build_string_from_cstring(myself->value.avalue->
							  immutable ? ", " :
							  " ; ");
			string = ctr_string_append(string, newArgumentList);
		}
...
ctr_object *ctr_array_column(ctr_object * myself, ctr_argument * argumentList)
{
	...
	for (i = myself->value.avalue->tail; i < myself->value.avalue->head;
		...
		if (n >= element->value.avalue->head)
			continue;
		newArgumentList->object =
		    *(element->value.avalue->elements +
		      element->value.avalue->tail + n);
...
ctr_object *ctr_map_put(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if (!hasher) {
		ctr_object *putKey;
		putKey =
		    ctr_send_message(nextArgument->object, "toString", 8, NULL);
		...
		ctr_internal_object_delete_property_with_hash(myself,
							      ...
							      *(uint64_t
								*) (&hashk), 0);
		ctr_internal_object_add_property_with_hash(myself,
							   nextArgument->object,
							   *(uint64_t
							     *) (&hashk),
							   putValue, 0);
...
ctr_object *ctr_map_rm(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if (!hasher) {
		putKey =
		    ctr_send_message(nextArgument->object, "toString", 8,
				     emptyArgumentList);
...
ctr_object *ctr_map_get(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if (!hasher) {
		...
		foundObject =
		    ctr_internal_object_find_property(myself, searchKey, 0);
		if (foundObject == NULL)
...
ctr_object *ctr_map_kvmap(ctr_object * myself, ctr_argument * argumentList)
{
	...
	while (m) {
		...
		if (arguments->object == block) {
			...
			arguments->next->object =
			    ctr_array_get(ret, larguments);
		}
		ctr_map_put(newmap, arguments);
		m = m->next;
	}
...
ctr_object *ctr_map_to_string(ctr_object * myself, ctr_argument * argumentList)
{
	...
	while (mapItem) {
		newArgumentList->object =
		    ctr_build_string_from_cstring(CTR_DICT_CODEGEN_MAP_PUT);
		ctr_string_append(string, newArgumentList);
		if (mapItem->value == myself) {
			...
			ctr_string_append(string, newArgumentList);
			   ...
			   || mapItem->value->info.type ==
			   CTR_OBJECT_TYPE_OTNUMBER
			   || mapItem->value->info.type ==
			   CTR_OBJECT_TYPE_OTNIL) {
			newArgumentList->object = mapItem->value;
			ctr_string_append(string, newArgumentList);
		} else if (mapItem->value->info.type ==
			   CTR_OBJECT_TYPE_OTSTRING) {
			newArgumentList->object =
			    ctr_build_string_from_cstring("'");
			ctr_string_append(string, newArgumentList);
			newArgumentList->object =
			    ctr_string_quotes_escape(mapItem->value,
						     newArgumentList);
			ctr_string_append(string, newArgumentList);
			newArgumentList->object =
			    ctr_build_string_from_cstring("'");
		...
		newArgumentList->object =
		    ctr_build_string_from_cstring(CTR_DICT_CODEGEN_MAP_PUT_AT);
		ctr_string_append(string, newArgumentList);
		if (mapItem->key->info.type == CTR_OBJECT_TYPE_OTBOOL
		    || mapItem->key->info.type == CTR_OBJECT_TYPE_OTNUMBER
			...
			ctr_string_append(string, newArgumentList);
			newArgumentList->object =
			    ctr_string_quotes_escape(mapItem->key,
						     newArgumentList);
			ctr_string_append(string, newArgumentList);
			newArgumentList->object =
			    ctr_build_string_from_cstring("'");
...
ctr_object *ctr_map_assign(ctr_object * myself, ctr_argument * argumentList)
{
	...
	while (mapItem) {
		if (mapItem->value == myself) {
			CtrStdFlow =
			    ctr_build_string_from_cstring
			    ("Assign cannot have selfReference.");
			return myself;
		}
		ctr_object *value;
		if (!
		    (value =
		     ctr_internal_object_find_property(to, mapItem->key, 0))) {
			mapItem = mapItem->next;
			continue;
		}
		value->info.raw = 1;
...
struct ctr_iters_type ctr_iterators =
    { &ctr_iter_range, &ctr_iter_urange, &ctr_iter_repeat };
...
ctr_object *ctr_iterator_to_string(ctr_object * myself,
				   ctr_argument * argumentList)
{
	char buf[10240];
 ...
 * Iterator set seed
 */
ctr_object *ctr_iterator_set_seed(ctr_object * myself,
				  ctr_argument * argumentList)
{
 ...
 */
ctr_object *ctr_iterator_set_func_(ctr_object * myself,
				   ctr_object * function, ctr_object * end,
				   ctr_object * step)
{
	ctr_object *func =
	    ctr_invoke_variadic(CtrStdReflect, &ctr_reflect_fn_copy, 1,
				function);

	ctr_internal_object_set_property(myself,
					 ctr_build_string_from_cstring("func"),
					 func, CTR_CATEGORY_PRIVATE_PROPERTY);
 ...
 */
ctr_object *ctr_iterator_set_func(ctr_object * myself,
				  ctr_argument * argumentList)
{
 ...
 *
 */
ctr_object *ctr_iterator_make_repeat(ctr_object * myself,
				     ctr_argument * argumentList)
{
	ctr_object *instance = ctr_iterator_make(myself, NULL);
	ctr_iterator_set_seed(instance, argumentList);
	ctr_iterator_set_func_(instance,
			       *ctr_iterators.ctr_iterator_repeat_func, NULL,
			       NULL);
	return instance;
}
 ...
 *
 */
ctr_object *ctr_iterator_make_range(ctr_object * myself,
				    ctr_argument * argumentList)
{
	...
	ctr_iterator_set_func_(instance, argo->object, argo->next->object,
			       argo->next->next->object);
 ...
 */
ctr_object *ctr_iterator_make_uncapped_range(ctr_object * myself,
					     ctr_argument * argumentList)
{
	ctr_argument *argo = argumentList;
	...
	ctr_iterator_set_func_(instance, argo->object, NULL,
			       argo->next->object);
...
ctr_object *ctr_iterator_next(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if (!iter) {
		CtrStdFlow =
		    ctr_build_string_from_cstring("Invalid iterator function.");
		return myself;
	}
	if (!seed) {
 ...
 * Runs the block for each value in the iterator while stepping through it
 */
ctr_object *ctr_iterator_each_v(ctr_object * myself,
				ctr_argument * argumentList)
{
	ctr_object *dothis = argumentList->object;
	ctr_argument *arg = ctr_heap_allocate(sizeof(ctr_argument));
	...
	ctr_object *iter = ctr_internal_object_find_property(myself,
							     ...
							     0);
	ctr_object *value =
	    ctr_internal_object_find_property(myself, seed_str, 0);
	int oldsticky = myself->info.sticky;
	myself->info.sticky = 1;
	while (1) {
		arg->object = value;
...
ctr_object *ctr_iterator_fmap(ctr_object * myself, ctr_argument * argumentList)
{
	...
	ctr_object *func =
	    ctr_string_eval(ctr_build_string_from_cstring
			    ("{^my map_func applyTo: my inner next.}"), NULL);
	ctr_internal_object_add_property(func,
					 ctr_build_string_from_cstring
					 ("map_func"), argumentList->object, 0);
	ctr_internal_object_set_property(func,
					 ctr_build_string_from_cstring("inner"),
					 myself, 0);
	ctr_internal_object_set_property(iter,
					 ctr_build_string_from_cstring("func"),
					 func, CTR_CATEGORY_PRIVATE_PROPERTY);
	return iter;
}
 ...
 * this function does not evaluate anything from the iterator.
 */
ctr_object *ctr_iterator_filter(ctr_object * myself,
				ctr_argument * argumentList)
{
 ...
 */
ctr_object *ctr_iterator_takewhile(ctr_object * myself,
				   ctr_argument * argumentList)
{
...
{
	ctr_object *iterend =
	    ctr_string_eval(ctr_build_string_from_cstring("{True break.}"),
			    NULL);
	return iterend;
}
 ...
 */
ctr_object *ctr_iterator_end_check(ctr_object * myself,
				   ctr_argument * argumentList)
{
	ctr_object *clause =
	    (ctr_block_run(argumentList->object, NULL, myself));
	if (CtrStdFlow) {
		return myself;
	}
 ...
 */
ctr_object *ctr_iterator_to_array(ctr_object * myself,
				  ctr_argument * argumentList)
{
...
ctr_object *ctr_iterator_skip(ctr_object * myself, ctr_argument * argumentList)
{
	...
	myself->info.sticky = oldsticky;
	ctr_heap_free(seed_str);
	return value ? value : ctr_internal_object_find_property(myself,
								 ctr_build_string_from_cstring
								 ("seed"), 0);
}
