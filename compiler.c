...
const char *ctr_ast_node_names[] = {
    "exprassignment", "exprmessage",     "unamessage",  "binmessage",
    "kwmessage",      "ltrstring",       "reference",   "ltrnum",
    "codeblock",      "returnfromblock", "immutable",   "(unknown)",
    "(unknown)",      "(unknown)",       "(unknown)",   "(unknown)",
    "(unknown)",      "(unknown)",       "(unknown)",   "(unknown)",
    "(unknown)",      "(unknown)",       "(unknown)",   "(unknown)",
    "(unknown)",      "paramlist",       "instrlist",   "",
    "endofprogram",   "nested",          "ltrbooltrue", "ltrboolfalse",
    "ltrnil",         "program"};
...
void ctr_ccomp_env_update_assign_charstar(environment **env, char *name,
...
int ctr_ccomp_node_indeterministic_level(ctr_tnode *node) {
  ...
  case CTR_AST_NODE_ENDOFPROGRAM:
    return lastcci = 0;
  case CTR_AST_NODE_PARAMLIST: { // if we've been handed this, we have started
                                 // parsing a block
  ...
  case CTR_AST_NODE_REFERENCE: {
    ...
    if (fy) {
      printf("environment had an entry for %.*s : %p (level %d)\n", fy->length,
             fy->binding, fy->value, fy->indeterministic_level);
  ...
  case CTR_AST_NODE_IMMUTABLE: {
    ...
    while (nn) {
      max_level =
          fmax(max_level, ctr_ccomp_rec_nodes_indeterministic_level(nn));
      nn = nn->next;
    }
  ...
  case CTR_AST_NODE_BINMESSAGE:
    return lastcci = ctr_ccomp_node_indeterministic_level(
               node->nodes->node); // argument
  case CTR_AST_NODE_KWMESSAGE:
    return lastcci = ctr_ccomp_rec_nodes_indeterministic_level(
               node->nodes); // all the arguments
  case CTR_AST_NODE_EXPRMESSAGE: {
    int rec =
        ctr_ccomp_node_indeterministic_level(node->nodes->node); // receiver
    int msgs = ctr_ccomp_node_indeterministic_level(
        node->nodes->next->node); // messages
    if (rec < 2 && msgs < 2) {
      // we know the value one way or the other
      return lastcci = 1; // we can recursively figure out the result of this
                          // expression
    } else
  ...
  case CTR_AST_NODE_CODEBLOCK: {
    ...
    ctr_ccomp_node_indeterministic_level(
        node->nodes->node); // paramlist is level 0, but we need to handle it
    int max = ctr_ccomp_rec_nodes_indeterministic_level(
        node->nodes->next); // get the worst of our instructions
...
ctr_ccomp_optimize_node_inplace(volatile ctr_tnode volatile **node) {
  ...
  case CTR_AST_NODE_NESTED: {
    ...
    return ctr_ccomp_optimize_node_inplace(
        &(nodes->node)); // get rid of the nesting
  }
  case CTR_AST_NODE_IMMUTABLE:
  case CTR_AST_NODE_KWMESSAGE:
  case CTR_AST_NODE_UNAMESSAGE:
  ...
  case CTR_AST_NODE_EXPRMESSAGE: {
    int deter;
    if ((deter = ctr_ccomp_node_indeterministic_level((*node)->nodes->node)) <
        2) {
      printf("proceeding optimization of %p\n", node);
...
int ctr_ccomp_print_series_raw_transform(ctr_tlistitem *item, char space,
                                         char *tr, char *to) {
  char i;
  char spacer[3];
  int trlen = strlen(tr);
...
ctr_object *ctr_ccomp_message(ctr_tnode *paramNode) {
  ...
  case CTR_AST_NODE_REFERENCE:
    ...
    if (can_be_self && (strncmp(receiverNode->value, "me",
  ...
  while (li->next) {
    ...
    if (argumentList) {
      ...
      while (1) {
        ...
        aItem->object = o;
        /* we always send at least one argument, note that if you want to modify
         * the argumentList, be sure to take this into account */
...
ctr_object *ctr_ccomp_assignment(ctr_tnode *node) {
  ...
  if (assignee->modifier == 1) {
    ...
    result = ctr_assign_value_to_my(
        ctr_build_string(assignee->value, assignee->vlen), x);
  } else if (assignee->modifier == 2) {
    printf("%.*s = ", assignee->vlen, assignee->value);
    x = ctr_ccomp_expr(value, &wasReturn);
    result = ctr_assign_value_to_local(
        ctr_build_string(assignee->value, assignee->vlen), x);
  } else if (assignee->modifier == 3) {
    printf("(this.%.*s) = ", assignee->vlen, assignee->value);
    x = ctr_ccomp_expr(value, &wasReturn);
    result = ctr_assign_value(ctr_build_string(assignee->value, assignee->vlen),
                              x); // Handle lexical scoping
  } else {
    printf("%.*s = ", assignee->vlen, assignee->value);
    x = ctr_ccomp_expr(value, &wasReturn);
    result =
        ctr_assign_value(ctr_build_string(assignee->value, assignee->vlen), x);
  }
  if (CtrStdFlow == NULL) {
    ctr_callstack_index--;
  }
...
ctr_object *ctr_ccomp_expr(ctr_tnode *node, char *wasReturn) {
  ...
  case CTR_AST_NODE_ENDOFPROGRAM:
    ...
    //        CtrStdFlow->value.svalue->vlen, stdout); printf("\n");
    //    }
