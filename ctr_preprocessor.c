...
char *
ctr_internal_readf (char *file_name, u_int64_t * total_size)
{
  char *prg;
  char ch;
  int prev;
  ...
  prg = malloc (real_size);	/* add 4 bytes, 3 for optional closing sequence verbatim mode and one lucky byte! */
...
char *
escapeables (char *str, u_int64_t realsize)
{
  char c;
...
char *
transform (char *prg, u_int64_t size)
{
