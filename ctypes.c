 ...
 */
ffi_type *
ctr_ctype_citron_object_try_infer_type (ctr_object * object)
{
...
void
ctr_run_function_ptr (ffi_cif * cif, void *ret, void *args[], void *function)
{
  if (!(function && cif))
    return;
...
void
ctr_ctypes_set_type (ctr_object * object, ctr_ctype type)
{
...
ctr_ctype
ctr_ctypes_rtypeof (ctr_object * object)
{
...
{
  char buf[1024];
  sprintf(buf, "<CTypes ushort at: %p value: %ui>", myself->value.rvalue->ptr, *(unsigned short*)myself->value.rvalue->ptr);
  return ctr_build_string_from_cstring(buf);
}
...
{				//OUCH, too big for literal citron number. consider bigger number?
  CTR_CREATE_CTOBJECT (object);
...
*/
ctr_object *
ctr_ctypes_make_packed (ctr_object * myself, ctr_argument * argumentList)
{				//packed: [int] count: [int]
...
ctr_object *
ctr_ctypes_packed_from (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_ctypes_packed_set (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_ctypes_cont_array_t *ptr = myself->value.rvalue->ptr;
  if (!ptr)
    return myself;
...
ctr_object *
ctr_ctypes_packed_get (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_ctypes_cont_array_t *ptr = myself->value.rvalue->ptr;
  if (!ptr)
    return myself;
...
ctr_object *
ctr_ctypes_packed_count (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_ctypes_cont_array_t *ptr = myself->value.rvalue->ptr;
  if (!ptr)
...
ctr_object *
ctr_ctypes_packed_size (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_ctypes_cont_array_t *ptr = myself->value.rvalue->ptr;
  if (!ptr)
    return ctr_build_number_from_float (0);
...
ctr_object *
ctr_ctypes_packed_type (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_ctypes_cont_array_t *ptr = myself->value.rvalue->ptr;
  if (!ptr)
    {
...
ssize_t
ctr_ctype_get_c_size (ctr_object * obj)
{
  ...
  if ((meta) == CtrStdCType_void)
    ...
    return ctr_ctypes_struct_get_size (obj, NULL)->value.nvalue;	//signal for struct
...
ctr_object *
ctr_ctypes_addr_of (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_ctypes_addr_of_raw (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_object *ret = ctr_ctypes_make_pointer (NULL, NULL);
  if (myself->info.type != CTR_OBJECT_TYPE_OTEX)
    {
...
ctr_object *
ctr_ctypes_pointer_set_obj (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_object *ret = ctr_ctypes_make_pointer (NULL, NULL);
  ret->value.rvalue->ptr = &(myself);
  return ret;
...
ctr_object *
ctr_ctypes_pointer_get_obj (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_ctypes_to_bytes_string (ctr_object * myself, ctr_argument * argumentList)
{
  char *buf = (void *) myself->value.rvalue->ptr;
  int count = ctr_internal_cast2number (argumentList->object)->value.nvalue;
...
ctr_object *
ctr_ctypes_read_string (ctr_object * myself, ctr_argument * argumentList)
{
  char *buf = (void *) myself->value.rvalue->ptr;
...
ctr_object *
ctr_ctype_new_instance (ctr_object * type)
{
...
ctr_object *
ctr_ctype_struct_copy (ctr_object * str)
{				//TODO: Write copy method for struct
  return str;
}

ctr_object *
ctr_ctypes_deref_pointer (ctr_object * myself, ctr_argument * argumentList)
{
  if (argumentList->object->info.type != CTR_OBJECT_TYPE_OTEX)
    {
...
ctr_object *
ctr_ctypes_set_internal_pointer (ctr_object * myself, ctr_argument * argumentList)
{
  if (argumentList->object->info.type != CTR_OBJECT_TYPE_OTEX)
    {
...
ctr_object *
ctr_ctypes_struct_make_internal (ctr_object * myself, char *_fmt)
{
  CTR_CREATE_CTOBJECT (object);
...
ctr_object *
ctr_ctypes_make_struct (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_ctypes_pack_struct (ctr_object * myself, ctr_argument * argumentList)
{				//<struct> pack: [member, ...]
...
ctr_object *
ctr_ctypes_pack_struct_at (ctr_object * myself, ctr_argument * argumentList)
{				//<struct> put: [value] at: [offset]
  if (myself->info.type != CTR_OBJECT_TYPE_OTEX || myself->value.rvalue->type != CTR_CTYPE_STRUCT)
    {
      //TODO: Error out
      printf ("Invalid object\n");
...
ctr_object *
ctr_ctypes_unpack_struct (ctr_object * myself, ctr_argument * argumentList)
{				//<struct> unpack
  if (myself->info.type != CTR_OBJECT_TYPE_OTEX || myself->value.rvalue->type != CTR_CTYPE_STRUCT)
    {
      //TODO: Error out
  ...
  for (; i < member_count; i++)
      ...
      if (!padinfo[init])
	{
      ...
      while (!ignore_pads && padinfo[init]->pad)
	{			//skip them pads
	  // fieldsp+=1;
...
ctr_object *
ctr_ctypes_unpack_struct_at (ctr_object * myself, ctr_argument * argumentList)
{				//<struct> atOffset:
...
ctr_object *
ctr_ctypes_struct_to_string (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_ctypes_struct_new (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_ctypes_ffi_struct_value *ptr = myself->value.rvalue->ptr;
...
ctr_object *
ctr_ctypes_unmake_struct (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_ctypes_struct_allocate (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_ctypes_struct_set_from (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_ctypes_struct_get_size (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_ctypes_ffi_struct_value *ptr = myself->value.rvalue->ptr;
...
ctr_object *
ctr_ctypes_struct_get_member_count (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_ctypes_ffi_struct_value *ptr = myself->value.rvalue->ptr;
...
ctr_object *
ctr_ctypes_struct_get_padding_format (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_ctypes_ffi_struct_value *ptr = myself->value.rvalue->ptr;
...
ctr_object *
ctr_ctypes_struct_get_offset_format (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_ctypes_ffi_struct_value *ptr = myself->value.rvalue->ptr;
  ctr_object *arr = ctr_array_new (CtrStdArray, NULL);
  ctr_argument *args = ctr_heap_allocate (sizeof (ctr_argument));
...
ctr_object *
ctr_ctypes_get_first_meta (ctr_object * object, ctr_object * last)
{
...
ffi_type *
ctr_ctypes_ffi_convert_to_ffi_type (ctr_object * type)
{
...
ctr_ctype
ctr_ctypes_ffi_convert_to_citron_ctype (ctr_object * type)
{
  ...
  else if ((initial) == CtrStdCType_pointer || initial == CtrStdCType_dynamic_lib)
...
ffi_type *
ctr_ctypes_ffi_convert_ctype_to_ffi_type (ctr_ctype type)
{
  switch (type)
    {
    case CTR_CTYPE_VOID:
...
ctr_ctype
ctr_ctypes_ffi_convert_ffi_type_to_ctype (ffi_type * type)
{
...
ctr_object *
ctr_ctypes_convert_ffi_type_to_citron (ffi_arg * value, ctr_ctype type)
{
  // printf("p %d\n", type);
...
{				//cif*, int<abi>, type* rtype, type** atypes
  ...
  if (status != FFI_OK)
    ...
    {
      ctr_internal_object_set_property (myself, ctr_build_string_from_cstring (":crType"),
					ctr_build_number_from_float (ctr_ctypes_ffi_convert_to_citron_ctype (argumentList->next->object)), 0);
    }
  return myself;
}

CTR_CT_FFI_BIND (prep_cif_inferred)
{				//[*atypes, rtype]
  ctr_object *cifobj = ctr_internal_create_object (CTR_OBJECT_TYPE_OTEX);
...
error_out_inf:;
  ...
  if (status != FFI_OK)
    ...
    {
      ctr_internal_object_set_property (cifobj, ctr_build_string_from_cstring (":crType"),
					ctr_build_number_from_float (ctr_ctypes_ffi_convert_ffi_type_to_ctype (rtype)), 0);
    }
  return cifobj;
}

CTR_CT_FFI_BIND (cif_arg_count)
{				//<cif; ^Nil|Number
...
{
  if (argumentList->object->info.type == CTR_OBJECT_TYPE_OTEX &&
      (argumentList->object->value.rvalue->type == CTR_CTYPE_POINTER || argumentList->object->value.rvalue->type == CTR_CTYPE_STRING))
    {
      myself->value.rvalue->ptr = argumentList->object->value.rvalue->ptr;
    }
...
{
  if (argumentList->object->info.type == CTR_OBJECT_TYPE_OTEX && argumentList->object->value.rvalue->type == CTR_CTYPE_FUNCTION_POINTER)
    {
      myself->value.rvalue->ptr = argumentList->object->value.rvalue->ptr;
    }
