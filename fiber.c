...
void fiberYield()
{
	...
	if (inFiber) {
		...
		LF_DEBUG_OUT
		    ("libfiber debug: Fiber %d yielding the processor...",
		     currentFiber);
...
int spawnFiber(ctr_object * block)
{
	...
	makecontext(&fiberList[numFibers].context, (void (*)(void))&fiberStart,
		    1, block);
	++numFibers;
...
ctr_object *ctr_fiber_spawn(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if (blk == NULL || blk->info.type != CTR_OBJECT_TYPE_OTBLOCK) {
		CtrStdFlow =
		    ...
		    ("Fiber's spawning requires a block.");
		return myself;
	}
...
void ctr_fiber_begin_init()
{
	...
	ctr_internal_create_func(CtrStdFiber,
				 ...
				 &ctr_fiber_join_all);
	ctr_internal_create_func(CtrStdFiber,
				 ctr_build_string_from_cstring("wait"),
				 &ctr_fiber_join);
	ctr_internal_create_func(CtrStdFiber,
