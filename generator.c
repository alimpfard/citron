  ...
  case CTR_OBJECT_TYPE_OTNUMBER: {
    ...
    if (argumentList->next && (end = argumentList->next->object)) { // from_to_
      ...
      if (argumentList->next->next &&
          (step = argumentList->next->next->object)) { // from_to_step
...
ctr_object *ctr_generator_next(ctr_object *myself, ctr_argument *argumentList) {
  ...
  if (!next) {
    CtrStdFlow =
        ctr_build_string_from_cstring("Invalid generator type(probably)");
    return CtrStdNil;
  }
...
ctr_object *ctr_generator_inext(ctr_object *myself,
  ...
  if (!genny) {
    CtrStdFlow =
        ctr_build_string_from_cstring("#inext on uninitialized generator");
    return CtrStdNil;
  }
...
ctr_generator *ctr_internal_generator_copy(ctr_generator *genny, int gtype) {
  ...
  case CTR_XFN_OF_GENNY: {
    *gcopy = *genny;
    ...
    gcopy->sequence = ctr_heap_allocate(sizeof(*mgen));
    ((ctr_mapping_generator *)(gcopy->sequence))->genny =
        ctr_internal_generator_copy(mgen->genny, mgen->i_type);
...
ctr_object *ctr_generator_underlaying(ctr_object *myself,
  ...
  int gtype = res->type;
  if (gtype != CTR_FN_OF_GENNY && gtype != CTR_IFN_OF_GENNY &&
      gtype != CTR_XFN_OF_GENNY) {
    CtrStdFlow = ctr_build_string_from_cstring(
