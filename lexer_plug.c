 ...
 */
static uint8_t
ctr_lex_is_delimiter (char symbol)
{
 ...
 */
void
ctr_lex_emit_error (char *message)
{
 ...
 */
void
ctr_lex_load (char *prg, ctr_size len)
{
  ctr_code = prg;
...
char *
ctr_lex_tok_describe (int token)
{
  char *description;
  switch (token)
    {
...
int
ctr_lex_is_valid_digit_in_base (char c, int b)
{
  if (b <= 10)
    {
      if ((c >= '0') && (c < ('0' + b)))
...
ctr_lex_tok ()
{
  ...
  if (ctr_lex_verbatim_mode == 1 && ctr_lex_verbatim_mode_insert_quote == (uintptr_t) ctr_code)
    {
  ...
  if (c == '\\')
	...
	{			//HACK This thing here returns a REF<\>, while the real lexer returns a BLOCKOPEN_MAP<>, and sets a flag.
	  ctr_lex_tokvlen = 1;
	  *ctr_lex_buffer = '\\';
  ...
  if ((c == '-' && (ctr_code + 1) < ctr_eofcode && isdigit (*(ctr_code + 1))) || isdigit (c))
      ...
      while ((ctr_lex_is_valid_digit_in_base (c, base)))
	{
      ...
      if (c == '.' && (ctr_code + 1 <= ctr_eofcode) && !ctr_lex_is_valid_digit_in_base (toupper (*(ctr_code + 1)), base))
	{
  ...
  while (!isspace (c) && (c != '#' &&
			     ...
			     && ((uint8_t) * (ctr_code + 2) == 145))) && (c != ':') && c != '\'') && ctr_code != ctr_eofcode)
    {
      ...
      if (i > ctr_lex_bflmt)
	{
	  ctr_lex_emit_error ("Token Buffer Exausted. Tokens may not exceed 255 bytes");
	}
      ctr_code++;
      if (ctr_code == ctr_eofcode)
	return CTR_TOKEN_REF;
  ...
  while ((			/* reading string in non-verbatim mode, read until the first non-escaped quote */
	   ctr_lex_verbatim_mode == 0 && (c != '\'' || escape == 1)) || (	/* reading string in verbatim mode, read until the '<?' sequence */
									  ...
									  (c
									   ...
									   && *(ctr_code + 1) == '?') && (ctr_code < ctr_eofcode)))
    {

      /* enter interpolation mode ( $$x ) */
      if (!ctr_lex_verbatim_mode && !escape && c == '$' && ((ctr_code + 1) < ctr_eofcode) && *(ctr_code + 1) == '$')
	{
	  int q = 2;
