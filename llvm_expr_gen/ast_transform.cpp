...
std::pair<llvm::Value*, std::tuple<argT, std::vector<std::tuple<llvm::Function*, std::vector<llvm::Value*>, int>>, Citron::Type>> Citron::NumberExprAST::codegen(bool intern) {
    return std::make_pair<llvm::Value*, std::tuple<argT, std::vector<std::tuple<llvm::Function*, std::vector<llvm::Value*>, int>>, Citron::Type>>(
        llvm::ConstantFP::get(llvmCoreValues::TheContext, llvm::APFloat(val)), 
        std::make_tuple<argT, std::vector<std::tuple<llvm::Function*, std::vector<llvm::Value*>, int>>, Citron::Type>(
            nullptr,
            {}, 
            Citron::Type::DummyTy
        )
...
std::pair<llvm::Value*, std::tuple<argT, std::vector<std::tuple<llvm::Function*, std::vector<llvm::Value*>, int>>, Citron::Type>> Citron::MessagesExpr::codegen(bool intern) {
    std::cout << " >> " << message_line << '\n';
    if(message_line.size() == 0) 
        return receiver->codegen(intern);
    Citron::Type last_type = receiver->ty();
    std::vector<std::tuple<llvm::Function*, std::vector<llvm::Value*>, int>> pipeline;
    for(int i=0; i<message_line.size(); i++) {
        ...
        llvm::Function* fn = std::get<1>(proto);
        ...
        for(int i=0, e=msg.arguments.size(); i<e; i++) {
            ...
            if(!v.first) {
                ...
                for(int vi=0, e=std::get<1>(vs).size(); vi<e; vi++) {
                    did_recurse = 2;
                    std::tuple<llvm::Function*, std::vector<llvm::Value*>, int> vf = std::get<1>(vs)[vi];
                    if(vi == 0) {
                        if (e == 1)
                            did_recurse = 1;
    ...
    llvm::IRBuilder<> b(llvmCoreValues::TheContext);
    b.SetInsertPoint(BB);
    llvm::Value* vp;
    ...
    for(decltype(pipeline)::iterator it = pipeline.begin(); it < pipeline.end(); it++) {
        ...
        vp = b.CreateCall(std::get<0>(*it), vec, "calltmp");
...
void Citron::llvmCoreValues::populateCache() {
    ...
    llvm::Function* fp = llvm::Function::Create(FT, llvm::Function::ExternalLinkage, "mplus_num", TheModule.get());
    MethodCache["+"] = {
...
int main() {
    ...
    static char s[] = "1234 + 4321 test: 345 - 2 this: 64 + 2.";
