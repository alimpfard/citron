...
ctr_tnode *
ctr_unmarshal_ast (char *stream, ctr_size avail, ctr_size * consumed)
{
  ...
  switch (type)
	...
	*consumed += len;
    ...
    case CTR_MARSH_AST_KWMESSAGE:
      {				//KWMESSAGE <msg_len> <message> <argument_count> (<argument>)+
...
void
ctr_marshal_ast (ctr_tnode * rnode, char **stream, ctr_size * stream_len, ctr_size * used)
{
  ...
  switch (rnode->type)
	...
	*(*stream + ((*used)++)) = CTR_MARSH_AST_EXPRASSIGNMENT;
    ...
    case CTR_AST_NODE_EXPRMESSAGE:
      {				//EXPRMESSAGE <receiver> <msg_node_count> (<message>)*
	...
	*used += sizeof (ctr_size);
    ...
    case CTR_AST_NODE_UNAMESSAGE:
      {				//UNAMESSAGE <message_len> <message_name>
	...
	*used += rnode->vlen;
    ...
    case CTR_AST_NODE_BINMESSAGE:
      {				//BINMESSAGE <msg_len(in case of ::)> <msg> <argument> (count = 1 implicit)
	...
	*used += rnode->vlen;
    ...
    case CTR_AST_NODE_REFERENCE:
      {				//REFERENCE <modifier> <length> <name>
	...
	*used += sizeof (ctr_size);
    ...
    case CTR_AST_NODE_CODEBLOCK:
      {				// CODEBLOCK (<argument>)* (<instruction>)*
...
void
ctr_marshal_meta_object (ctr_object * obj, char **stream, ctr_size * stream_len, ctr_size * used)
{				//don't pass bullshit
...
void
ctr_marshal_keyvalues (ctr_mapitem * head, ctr_size hlen, char **stream, ctr_size * stream_len, ctr_size * used)
{
  if (!*stream)
    {
...
void
ctr_marshal_store_link (ctr_object * obj, char **stream, ctr_size * stream_len, ctr_size * used)
{
  if (obj == CtrStdObject)
...
ctr_object *
ctr_marshal_resolve_linq (char *stream, ctr_size * stream_len, ctr_size * used)
{
...
ctr_size
ctr_marshal_object (ctr_object * obj, char **stream, ctr_size * stream_len /* in char* */ , ctr_size * used)
{				//We will allocate stream if it's NULL, or use it as-is (will assume the size fits)
  if (!*stream)
    {
      *stream = ctr_heap_allocate (sizeof (char) * 1024);
      *stream_len = 1024;
...
ctr_map *
ctr_unmarshal_keyvalues (char *stream, ctr_size * avail, ctr_size * consumed)
{
...
void
ctr_unmarshal_meta_object_into (ctr_object * obj, char *stream, ctr_size * avail, ctr_size * consumed)
{
...
ctr_object *
ctr_unmarshal_object (char *stream, ctr_size * avail, ctr_size * consumed)
{
  char type = *(stream + *consumed);
  (*consumed)++;
...
ctr_object *
ctr_unmarshal (char *object, ctr_size len)
{
  ctr_size c = 0, l = len;
...
ctr_object *
ctr_reflect_marshal (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_reflect_unmarshal (ctr_object * myself, ctr_argument * argumentList)
{
