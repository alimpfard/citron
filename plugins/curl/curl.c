 ...
 * @internal
 *
 * Send `type` message to object and return citron string
 *
 **/
 ...
 * [Curl] cleanup.
 *
 * Destroy Curl handle 'n resource
 *
 **/
...
ctr_object* ctr_curl_respondto(ctr_object* myself, ctr_argument* argumentList) {
	...
	switch (argObj->info.type) {
		...
		case CTR_OBJECT_TYPE_OTARRAY: {
			...
			ctr_argument* arg_ = ctr_heap_allocate(sizeof(ctr_argument));
			int e = ctr_array_count(argObj, NULL)->value.nvalue;
			for(int i=0;i<e;i++) {
				arg_->object = ctr_build_number_from_float(i);
				char* cs = ctr_heap_allocate_cstring(ctr_internal_cast2string(ctr_array_get(argObj, arg_)));
				// printf("%s\n", cs);
				list = curl_slist_append(list, cs);
				ctr_heap_free(cs);
			}
			ctr_heap_free(arg_);
			val = list;
