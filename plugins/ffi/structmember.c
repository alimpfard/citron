...
static int get_double(ctr_object* o, double* p) { //TODO: Error handling
  double x;
  if(o->info.type != CTR_OBJECT_TYPE_OTNUMBER) return -1;
  x = o->value.nvalue;
  *p = x;
  return 0;
}
...
struct npdt_t get_pentry(ffi_type* type) {
  ...
  while(npdtable[i].type!=NULL) {
    if(npdtable[i].type == type) return npdtable[i];
    i++;
  }
...
struct nudt_t get_uentry(ffi_type* type) {
  ...
  while(nudtable[i].type!=NULL) {
    if(nudtable[i].type == type) return nudtable[i];
    i++;
  }
...
int npchar(char* p, ctr_object* o) {
  if (o->info.type != CTR_OBJECT_TYPE_OTSTRING) return -1;
  if(o->value.svalue->vlen != 1) {
    char err[2048];
    int errlen = sprintf(err, "char requires string of length 1 but we were passed %.*s (%d chars)", o->value.svalue->vlen, o->value.svalue->value, o->value.svalue->vlen);
    CtrStdFlow = ctr_build_string(err, errlen);
    return -1;
  }
...
int npuchar(char* p, ctr_object* o) {
  ...
  if(o->value.svalue->vlen != 1) {
    char err[2048];
    int errlen = sprintf(err, "uchar requires string of length 1 but we were passed %.*s (%d chars)", o->value.svalue->vlen, o->value.svalue->value, o->value.svalue->vlen);
    CtrStdFlow = ctr_build_string(err, errlen);
    return -1;
  }
