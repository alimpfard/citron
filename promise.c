...
again:
  ...
  case CTR_PROMISE_MESSAGE:
    result =
        ctr_send_message_blocking(result, c_ent->target.message.value,
                                  c_ent->target.message.vlen, c_ent->arguments);
    break;
  }
  ctr_load_context(octx);
  // printf("%p -> %s\n", result,
  // ctr_heap_allocate_cstring(ctr_internal_cast2string(result)));
...
ctr_object *ctr_promise_make(ctr_object *myself, ctr_object *target,
                             ctr_argument *argumentList, ctr_object *receiver) {
  ...
  argumentList = cargs;
  ctr_object *instance =
      ctr_internal_create_standalone_object(CTR_OBJECT_TYPE_OTMISC);
  ...
  if (err) {
    CtrStdFlow = ctr_format_str(
        "ECreation of mutex failed for promise with error: %d", err);
    return CtrStdNil;
  }
  err = pthread_mutex_init(sentry.return_lock, NULL);
  if (err) {
    CtrStdFlow = ctr_format_str(
        "ECreation of return mutex failed for promise with error: %d", err);
    return CtrStdNil;
  }
  ...
  *entry = sentry;
  ...
  if ((err = pthread_create(&entry->thread, NULL, &ctr_promise_thread_sf,
                            entry))) {
    CtrStdFlow = ctr_format_str(
        "ECreation of thread failed for promise with error: %d", err);
    return CtrStdNil;
  }
  return instance;
}
...
ctr_object *ctr_promise_dummy(ctr_object *myself, ctr_argument *argumentList) {
  ...
  return ctr_send_message_async(ctr_build_string_from_cstring("Dummy"),
                                "skip:", 5, args);
}
...
ctr_object *ctr_promise_q_finished(ctr_object *myself,
                                   ctr_argument *argumentList) {
  struct ctr_promise_entry *entry = myself->value.rvalue->ptr;
  pthread_mutex_unlock(entry->return_lock);
  ctr_object *s = ctr_build_bool(entry->discardable & 2);
  printf("Thread %x has ready status %s\n", entry->thread,
         entry->discardable & 2 ? "finished" : "busy");
  pthread_mutex_lock(entry->return_lock); // can't unlock if it's not locked
  return s;
}

ctr_object *ctr_promise_make_async(ctr_object *myself,
                                   ctr_argument *argumentList) {
  myself->info.asyncMode = 1;
  return myself;
}

ctr_object *ctr_promise_set_disc(ctr_object *myself,
                                 ctr_argument *argumentList) {
  struct ctr_promise_entry *entry = myself->value.rvalue->ptr;
  entry->discardable |= 1;
  return myself;
...
void promise_begin() {
  ...
  ctr_internal_create_func(
      CtrStdPromise, ctr_build_string_from_cstring("then:"), ctr_promise_then_);
  ctr_internal_create_func(
      CtrStdPromise, ctr_build_string_from_cstring("await"), ctr_promise_wait);
  ctr_internal_create_func(CtrStdPromise,
                           ctr_build_string_from_cstring("discard"),
                           ctr_promise_set_disc);
  ctr_internal_create_func(CtrStdPromise,
                           ctr_build_string_from_cstring("finished?"),
                           ctr_promise_q_finished);
  ctr_internal_create_func(CtrStdPromise,
                           ctr_build_string_from_cstring("toString"),
                           ctr_object_to_string);
  ctr_internal_create_func(CtrStdPromise,
                           ctr_build_string_from_cstring("respondTo:"),
                           ctr_promise_pass_message);
  ctr_internal_create_func(CtrStdPromise,
                           ctr_build_string_from_cstring("respondTo:and:"),
                           ctr_promise_pass_message);
  ctr_internal_create_func(CtrStdPromise,
                           ctr_build_string_from_cstring("respondTo:and:and:"),
                           ctr_promise_pass_message);
  ctr_internal_create_func(
      CtrStdPromise, ctr_build_string_from_cstring("respondTo:and:and:and:"),
      ctr_promise_pass_message);
  ctr_internal_create_func(
      CtrStdPromise,
      ctr_build_string_from_cstring("respondTo:and:and:and:and:"),
      ctr_promise_pass_message);
  ctr_internal_create_func(CtrStdObject, ctr_build_string_from_cstring("async"),
                           ctr_promise_make_async);
}
