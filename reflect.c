...
ctr_reflect_add_glob (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  ctr_internal_object_add_property (CtrStdWorld,
				    ctr_internal_cast2string (argumentList->object), instance, 0);
  return instance;
}
...
ctr_reflect_add_local (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  ctr_internal_object_add_property (ctr_contexts[ctr_context_id],
				    ...
				    (argumentList->object), instance, CTR_CATEGORY_PUBLIC_PROPERTY);
  return CtrStdNil;
}
  ...
  while ((param = parameterList->node) != NULL)
    {
      args->object = (parameterList->next == NULL
		      ...
		      '*') ? ctr_build_string (param->value + 1,
					       param->vlen -
					       1) : ctr_build_string (param->value, param->vlen);
...
ctr_reflect_cb_ac_ (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  while ((param = parameterList->node) != NULL)
    {
      args->object = (parameterList->next == NULL
		      && *(param->value) == '*') ? ctr_build_string_from_cstring ("*Binding")
	...
	case CTR_AST_NODE_REFERENCE:
	  {
	    ...
	    ctr_invoke_variadic (retval, &ctr_string_append,
				 1, ctr_build_string_from_cstring (t->value));
	    break;
	  }
  ...
  while (nodes && (node = nodes->node) != NULL)
      ...
      if (node->value)
	{
	  ...
	  if (node->type == CTR_AST_NODE_EXPRMESSAGE
	      ...
	      || node->type == CTR_AST_NODE_BINMESSAGE || node->type == CTR_AST_NODE_UNAMESSAGE)
	    {
...
ctr_reflect_type_descriptor_print (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  switch (object->info.type)
    ...
    case CTR_OBJECT_TYPE_OTBLOCK:
      type_descriptor =
	ctr_invoke_variadic (myself,
			     &ctr_reflect_type_descriptor_print, 1,
...
ctr_reflect_describe_type (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  switch (object->info.type)
	...
	ctr_string_eval (ctr_build_string_from_cstring
			 ("{:kv ^[(kv @ 0), (Reflect typeOf: kv @ 1)].}"), NULL);
...
ctr_reflect_describe_value (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  switch (object->info.type)
    ...
    case CTR_OBJECT_TYPE_OTOBJECT:
      ...
      ctr_reflect_map_type_descriptor =
	ctr_string_eval (ctr_build_string_from_cstring ("{:kv ^[(kv @ 0), (kv @ 1)].}"), NULL);
      arg->object = ctr_reflect_map_type_descriptor;
...
int
ctr_reflect_check_bind_valid (ctr_object * from, ctr_object * to)
{
  ...
  if (!(ctr_internal_object_is_constructible_ (from_type, to_type, to->info.raw)))
    {
      ...
      argumentList->object =
	ctr_string_replace_with (ctr_internal_cast2string (to_type), argumentList);
...
ctr_reflect_get_first_link (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  if (link)
    ...
    {
      CtrStdFlow =
	ctr_build_string_from_cstring ("Attempt to get parent of native object (NULL deref)");
      return CtrStdNil;
    }
}
  ...
  if (methodObject)
    {
      ctr_object *meth =
	ctr_string_eval (ctr_build_string_from_cstring
			 ("{:self:*args ^Reflect run: my method forObject: self arguments: args.}"),
			 NULL);
      ctr_internal_object_add_property (meth,
					ctr_build_string_from_cstring ("method"), methodObject, 0);
      return meth;
    }
  return CtrStdNil;
}
...
ctr_reflect_closure_of (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  ctr_internal_object_set_property (methodObject,
				    ctr_build_string_from_cstring
				    ("method"), methodName, CTR_CATEGORY_PRIVATE_PROPERTY);
  return methodObject;
}
...
ctr_reflect_get_property (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  sprintf (buf,
	   ...
	   ns, (int) (ns_type->value.svalue->vlen), ns_type->value.svalue->value);
