		...
		case CTR_AST_NODE_EXPRASSIGNMENT:
			{
				...
				ctr_object *assignee =
				    ctr_reflect_get_nodes(t->nodes->node, ns);
				ctr_object *assigned =
				    ctr_reflect_get_nodes(t->nodes->next->node,
							  ns);
				ctr_argument *args =
				    ctr_heap_allocate(sizeof(ctr_argument));
				args->object =
				    ctr_build_string_from_cstring("assign");
		...
		case CTR_AST_NODE_CODEBLOCK:
			{
				...
				ctr_object *receiver =
				    ctr_reflect_cb_ac(CtrStdReflect, args);
				ctr_object *message =
				    ctr_reflect_get_nodes(t->nodes->next->node,
							  ns);
				args->object =
				    ctr_build_string_from_cstring("block");
				ctr_array_push(retval, args);
				args->object = receiver;
				ctr_array_push(retval, args);
				args->object = message;
		...
		case CTR_AST_NODE_PARAMLIST:
			{
				...
				while ((param = parameterList->node) != NULL) {
					args->object =
					    ctr_build_string(param->value,
							     param->vlen);
					ctr_send_message(retval, "push:", 5,
							 args);
					parameterList = parameterList->next;
					if (parameterList == NULL)
						break;
		...
		case CTR_AST_NODE_INSTRLIST:
			{
				...
				while (1) {
					if (!instrli->node)
						break;
					args->object =
					    ctr_reflect_get_nodes(instrli->node,
								  ns);
					ctr_array_push(retval, args);
					if (!instrli->next)
						break;
					...
					ctr_argument * burrowed, int i)
{
	...
	while (nodes && (node = nodes->node) != NULL) {
		...
		if (node->value) {
			burrowed->object =
			    ctr_build_string(node->value, node->vlen);
			if (node->type == CTR_AST_NODE_EXPRMESSAGE
				...
				burrowed->object =
				    ctr_reflect_serialize_expr_(node, burrowed,
								1);
				     ...
				     ctr_argument * argumentList)
{
	...
	if (parameterList == NULL) {
		...
		parameterList->node =
		    ctr_heap_allocate_tracked(sizeof(ctr_tnode));
		char *name =
		    ctr_heap_allocate_cstring(argumentList->next->object);
		parameterList->node->value =
		    ctr_heap_allocate_tracked(sizeof(name));
				...
				ctr_argument * argumentList)
{
	...
	if (argumentList->object->info.type != CTR_OBJECT_TYPE_OTBLOCK) {
		CtrStdFlow =
		    ctr_build_string_from_cstring
		    ("parameter was of an invalid type.");
		return CtrStdNil;
	}
					      ...
					      ctr_argument * argumentList)
{
	...
	case CTR_OBJECT_TYPE_OTNATFUNC:
		type_descriptor =
		    ctr_build_string_from_cstring("[NativeFunction]");
		break;
	...
	case CTR_OBJECT_TYPE_OTARRAY:
		...
		for (; i < c; i++) {
			...
			arg->object =
			    ctr_reflect_type_descriptor_print(myself, arg);
					     ...
					     ctr_argument * argumentList)
{
	if (argumentList->object->info.type == CTR_OBJECT_TYPE_OTSTRING) {
		ctr_object *desc =
		    ctr_build_string_from_cstring("String(length=");
		argumentList->object =
				      ...
				      ctr_argument * argumentList)
{
	...
	if (methodObject
	    && methodObject->info.type == CTR_OBJECT_TYPE_OTNATFUNC) {
		ctr_object *mn =
		    ctr_build_string_from_cstring
...
ctr_object *ctr_reflect_object_delegate_get_responder(ctr_object * itself,
						      ctr_argument *
						      argumentList)
{
	ctr_argument *arg = ctr_heap_allocate(sizeof(ctr_argument));
...
ctr_object *ctr_reflect_run_for_object_in_ctx(ctr_object * myself,
					      ctr_argument * argumentList)
{
	ctr_object *block = argumentList->object, *ctx =
	    argumentList->next->object, *arguments =
