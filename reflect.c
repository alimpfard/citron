...
ctr_object *
ctr_reflect_new (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_reflect_add_glob (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_reflect_add_local (ctr_object * myself, ctr_argument * argumentList)
{
  CTR_ENSURE_TYPE_STRING (argumentList->object);
  ctr_object *instance = ctr_internal_create_object (CTR_OBJECT_TYPE_OTOBJECT);
  ctr_set_link_all (instance, CtrStdObject);
  ctr_internal_object_add_property (ctr_contexts[ctr_context_id],
				    ctr_internal_cast2string (argumentList->object), instance, CTR_CATEGORY_PUBLIC_PROPERTY);
  return CtrStdNil;
}
 ...
 */
ctr_object *
ctr_reflect_add_my (ctr_object * myself, ctr_argument * argumentList)
{
  CTR_ENSURE_TYPE_STRING (argumentList->object);
  ctr_object *instance = ctr_internal_create_object (CTR_OBJECT_TYPE_OTOBJECT);
  ctr_set_link_all (instance, CtrStdObject);
  ctr_internal_object_add_property (ctr_contexts[ctr_context_id],
				    ctr_internal_cast2string (argumentList->object), instance, CTR_CATEGORY_PRIVATE_PROPERTY);
  return CtrStdNil;
}
 ...
 */
ctr_object *
ctr_reflect_set_to (ctr_object * myself, ctr_argument * argumentList)
{
  // CTR_ENSURE_TYPE_STRING(argumentList->object);
 ...
 */
ctr_object *
ctr_reflect_dump_context (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_reflect_dump_context_spec (ctr_object * myself, ctr_argument * argumentList)
{
...
int
ctr_internal_has_own_responder (ctr_object * of, ctr_object * meth)
{
  int m = of->methods->size - 1;
  struct ctr_mapitem *head;
  head = of->methods->head;
...
ctr_object *
ctr_reflect_has_own_responder (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_object *of = argumentList->object;
 ...
 */
ctr_object *
ctr_reflect_dump_context_spec_prop (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_object *of = argumentList->object;
 ...
 */
ctr_object *
ctr_reflect_dump_context_prop (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_object *of = argumentList->object;
 ...
 **/
ctr_object *
ctr_reflect_find_obj (ctr_object * myself, ctr_argument * argumentList)
{
  CTR_ENSURE_TYPE_STRING (argumentList->object);
 ...
 */
ctr_object *
ctr_reflect_extst_obj (ctr_object * myself, ctr_argument * argumentList)
{
  CTR_ENSURE_TYPE_STRING (argumentList->object);
  int i = ctr_context_id;
...
ctr_reflect_cb_ac (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  while ((param = parameterList->node) != NULL)
    {
      args->object = (parameterList->next == NULL
		      && *(param->value) ==
		      '*') ? ctr_build_string (param->value + 1, param->vlen - 1) : ctr_build_string (param->value, param->vlen);
...
ctr_object *
ctr_reflect_cb_ac_ (ctr_object * myself, ctr_argument * argumentList)
{
  CTR_ENSURE_TYPE_BLOCK (argumentList->object);
  ctr_tnode *obj = argumentList->object->value.block;
  if (obj == NULL)
  ...
  while ((param = parameterList->node) != NULL)
    {
      args->object = (parameterList->next == NULL
		      && *(param->value) == '*') ? ctr_build_string_from_cstring ("*Binding") : ctr_build_string_from_cstring ("Binding");
      ctr_send_message (arglist, "push:", 5, args);
      parameterList = parameterList->next;
      if (parameterList == NULL)
	break;
...
#ifdef UNPARSE
ctr_object *
ctr_reflect_get_nodes (ctr_tnode * ti, ctr_object * ns)
{
  ctr_tlistitem *li;
  ctr_tnode *t;
  li = ti->nodes;
  ...
  while (1)
	...
	case CTR_AST_NODE_EXPRASSIGNMENT:
	  {
	    ...
	    ctr_object *assigned = ctr_reflect_get_nodes (t->nodes->next->node,
							  ns);
	    ctr_argument *args = ctr_heap_allocate (sizeof (ctr_argument));
	...
	case CTR_AST_NODE_EXPRMESSAGE:
	  {
	    ...
	    ctr_object *message = ctr_reflect_get_nodes (t->nodes->next->node,
							 ns);
	    ctr_argument *args = ctr_heap_allocate (sizeof (ctr_argument));
	...
	case CTR_AST_NODE_KWMESSAGE:
	  {			//TODO: Unparse all argument nodes
	    retval = ctr_array_new (CtrStdArray, NULL);
	    ctr_object *msgargs = ctr_array_new (CtrStdArray, NULL);
	    ctr_object *name = ctr_build_string (t->value, t->vlen);
	    ctr_argument *args = ctr_heap_allocate (sizeof (ctr_argument));
...
ctr_object *
ctr_reflect_dump_function (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_reflect_unparse (ctr_object * arr, int type)
{
...
ctr_object *
ctr_reflect_serialize_expr (ctr_tnode * expr_node, ctr_argument * b)
{
  return ctr_reflect_serialize_expr_ (expr_node, b, 0);
}

ctr_object *
ctr_reflect_serialize_expr_ (ctr_tnode * expr_node, ctr_argument * burrowed, int i)
{
  ...
  while (nodes && (node = nodes->node) != NULL)
      ...
      if (node->value)
	{
	  ...
	  if (node->type == CTR_AST_NODE_EXPRMESSAGE
	      || node->type == CTR_AST_NODE_KWMESSAGE || node->type == CTR_AST_NODE_BINMESSAGE || node->type == CTR_AST_NODE_UNAMESSAGE)
	    {
...
ctr_object *
ctr_reflect_cb_ic (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_tnode *obj = argumentList->object->value.block;
  if (obj == NULL)
    return CtrStdNil;
 ...
 */
ctr_object *
ctr_reflect_cb_add_param (ctr_object * myself, ctr_argument * argumentList)
{
  CTR_ENSURE_TYPE_BLOCK (argumentList->object);
 ...
 */
ctr_object *
ctr_reflect_fn_copy (ctr_object * myself, ctr_argument * argumentList)
{
  CTR_ENSURE_TYPE_BLOCK (argumentList->object);
  ...
  ctr_object *prop = ctr_internal_object_find_property (argumentList->object, name,
							CTR_CATEGORY_PRIVATE_PROPERTY);
  if (prop != NULL)
    ctr_internal_object_add_property (newblk, name, prop, CTR_CATEGORY_PRIVATE_PROPERTY);
 ...
 */
ctr_tnode *
ctr_reflect_internal_term_rewrite (ctr_tnode * pfn, int nodety, ctr_tnode * rewrite_term, int replace_mask)
{
 ...
 **/
ctr_object *
ctr_reflect_share_memory (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 **/
ctr_object *
ctr_reflect_link_to (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 **/
ctr_object *
ctr_reflect_find_obj_ex (ctr_object * myself, ctr_argument * argumentList)
{
...
int
ctr_reflect_is_linked_to_ (ctr_argument * argumentList)
{
 ...
 **/
ctr_object *
ctr_reflect_is_linked_to (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 **/
ctr_object *
ctr_reflect_child_of (ctr_object * myself, ctr_argument * argumentList)
{
  return ctr_build_bool (argumentList->object->interfaces->link == argumentList->next->object);
}
 ...
 **/
ctr_object *
ctr_reflect_generate_inheritance_tree (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_reflect_type_descriptor_print (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  switch (object->info.type)
    ...
    case CTR_OBJECT_TYPE_OTBLOCK:
      type_descriptor =
	ctr_invoke_variadic (myself, &ctr_reflect_type_descriptor_print, 1, ctr_invoke_variadic (myself, &ctr_reflect_cb_ac_, 1, object));
...
ctr_object *
ctr_reflect_get_primitive_link (ctr_object * object)
{
 ...
 **/
ctr_object *
ctr_reflect_get_primitive (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 **/
ctr_object *
ctr_reflect_describe_type (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_object *object = argumentList->object;
  ...
  switch (object->info.type)
    ...
    case CTR_OBJECT_TYPE_OTOBJECT:
      ...
      ctr_reflect_map_type_descriptor = ctr_string_eval (ctr_build_string_from_cstring ("{:kv ^[(kv @ 0), (Reflect typeOf: kv @ 1)].}"), NULL);
...
ctr_object *
ctr_reflect_describe_value (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_object *object = argumentList->object;
  ctr_object *type_descriptor = CtrStdNil;
  ctr_argument *arg = ctr_heap_allocate (sizeof (ctr_argument));
  ...
  switch (object->info.type)
    ...
    case CTR_OBJECT_TYPE_OTOBJECT:
      ...
      ctr_reflect_map_type_descriptor = ctr_string_eval (ctr_build_string_from_cstring ("{:kv ^[(kv @ 0), (kv @ 1)].}"), NULL);
      arg->object = ctr_reflect_map_type_descriptor;
...
ctr_object *
ctr_reflect_describe_type_pretty (ctr_object * myself, ctr_argument * argumentList)
{
...
int
ctr_reflect_is_valid_ctor (ctr_object * candidate)
{
  ctr_argument *arg = ctr_heap_allocate (sizeof (ctr_argument));
...
int
ctr_reflect_check_bind_valid (ctr_object * from, ctr_object * to, int err_)
{
  ctr_object *err;
...
ctr_object *
ctr_reflect_check_bind_valid_v (ctr_object * myself, ctr_argument * argumentList)
{
  return ctr_build_bool (ctr_reflect_check_bind_valid (argumentList->object, argumentList->next->object, 0));
}

ctr_object *
ctr_reflect_bind (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_reflect_cons_of (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_reflect_try_serialize_block (ctr_object * myself, ctr_argument * argumentList)
{
  CTR_ENSURE_TYPE_BLOCK (argumentList->object);
  ctr_object *obj = argumentList->object;
 ...
 **/
ctr_object *
ctr_reflect_get_first_link (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_object *link = argumentList->object->interfaces->link;
  if (link)
    return link;
 ...
 **/
ctr_object *
ctr_reflect_get_responder (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  if (methodObject)
    {
      ctr_object *meth = ctr_string_eval (ctr_build_string_from_cstring ("{:self:*args ^Reflect run: my method forObject: self arguments: args.}"),
					  NULL);
...
ctr_object *
ctr_reflect_object_delegate_get_responder (ctr_object * itself, ctr_argument * argumentList)
{
  ctr_argument *arg = ctr_heap_allocate (sizeof (ctr_argument));
...
ctr_object *
ctr_reflect_object_get_responder (ctr_object * itself, ctr_argument * argumentList)
{
  if (argumentList == NULL || argumentList->next == NULL)
    CTR_ERR ("Argument cannot be NULL.");
  CTR_ENSURE_TYPE_STRING (argumentList->object);
 ...
 **/
ctr_object *
ctr_reflect_run_for_object (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_reflect_run_for_object_in_ctx_as_world (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_reflect_run_for_object_in_ctx_as_main (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 **/
ctr_object *
ctr_reflect_run_for_object_ctx (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_object *blk, *me, *argl;
  CTR_ENSURE_TYPE_BLOCK ((blk = argumentList->object));
  CTR_ENSURE_NON_NULL (argumentList->next);
 ...
 **/
ctr_object *
ctr_reflect_closure_of (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_object *methodName = argumentList->object;
  ctr_object *object = argumentList->next->object;
  ctr_object *methodObject = ctr_string_eval (ctr_build_string_from_cstring ("{:*args ^my self message: my method arguments: args.}"),
					      NULL);
 ...
 **/
ctr_object *
ctr_reflect_get_property (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  sprintf (buf,
	   "Attempt to read nonexistent property '%.*s' of Object %p (namely '%.*s' in this context)",
	   (int) (name->value.svalue->vlen), name->value.svalue->value, ns, (int) (ns_type->value.svalue->vlen), ns_type->value.svalue->value);
 ...
 */
ctr_object *
ctr_reflect_rawmsg (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_reflect_instrmsg (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_reflect_register_instrumentor (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_internal_object_add_property_with_hash (ctr_instrumentor_funcs, argumentList->next->object,
					      ctr_send_message (argumentList->next->object, "iHash", 5, NULL)->value.nvalue, argumentList->object, 0);
  return myself;
}
 ...
 */
ctr_object *
ctr_reflect_unregister_instrumentor (ctr_object * myself, ctr_argument * argumentList)
{
  ctr_internal_object_delete_property_with_hash (ctr_instrumentor_funcs, argumentList->object,
 ...
 */
ctr_object *
ctr_reflect_run_glob (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_reflect_run_in_new_ctx (ctr_object * myself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_reflect_compilerinfo (ctr_object * myself, ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_reflect_delegate_set_private_property (ctr_object * itself, ctr_argument * argumentList)
{
...
ctr_object *
ctr_reflect_cons_str (ctr_object * myself, ctr_argument * argumentList)
{
