...
ctr_reflect_add_glob (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  if (ctr_internal_object_find_property
      (CtrStdWorld, argumentList->object, 0))
 ...
 */
ctr_object *
ctr_reflect_dump_context_spec (ctr_object * myself,
...
ctr_object *
ctr_reflect_has_own_responder (ctr_object * myself,
			       ctr_argument * argumentList)
{
  ctr_object *of = argumentList->object;
 ...
 */
ctr_object *
ctr_reflect_dump_context_spec_prop (ctr_object * myself,
				    ctr_argument * argumentList)
{
  ctr_object *of = argumentList->object;
	...
	case CTR_AST_NODE_EXPRASSIGNMENT:
	  {
	    ...
	    ctr_object *assigned =
	      ctr_reflect_get_nodes (t->nodes->next->node, ns);
	    ctr_argument *args = ctr_heap_allocate (sizeof (ctr_argument));
...
ctr_object *
ctr_reflect_serialize_expr_ (ctr_tnode * expr_node, ctr_argument * burrowed,
			     int i)
{
      ...
      if (node->value)
	{
	  ...
	  if (node->type == CTR_AST_NODE_EXPRMESSAGE
	      ...
	      || node->type == CTR_AST_NODE_UNAMESSAGE)
	    {
	      ...
	      burrowed->object =
		ctr_reflect_serialize_expr_ (node, burrowed, 1);
...
ctr_reflect_fn_copy (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  ctr_object *prop =
    ctr_internal_object_find_property (argumentList->object, name,
 ...
 **/
ctr_object *
ctr_reflect_generate_inheritance_tree (ctr_object * myself,
				       ctr_argument * argumentList)
{
  ...
  while (parent != NULL)
    {
      newarg->object =
	(parent->lexical_name == NULL ? parent : parent->lexical_name);
...
ctr_object *
ctr_reflect_type_descriptor_print (ctr_object * myself,
				   ctr_argument * argumentList)
{
  ...
  switch (object->info.type)
    ...
    case CTR_OBJECT_TYPE_OTBLOCK:
      type_descriptor =
	ctr_invoke_variadic (myself, &ctr_reflect_type_descriptor_print, 1,
			     ctr_invoke_variadic (myself, &ctr_reflect_cb_ac_,
						  1, object));
      type_descriptor =
...
ctr_object *
ctr_reflect_describe_type_pretty (ctr_object * myself,
				  ctr_argument * argumentList)
{
...
ctr_reflect_check_bind_valid (ctr_object * from, ctr_object * to)
{
  ...
  if (!
    ...
    {
      CtrStdFlow =
	ctr_build_string_from_cstring ("Cannot bind object of type ");
      argumentList->object = from;
      from_type =
	ctr_reflect_describe_type_pretty (CtrStdReflect, argumentList);
...
ctr_object *
ctr_reflect_try_serialize_block (ctr_object * myself,
				 ctr_argument * argumentList)
{
  CTR_ENSURE_TYPE_BLOCK (argumentList->object);
  ctr_object *obj = argumentList->object;
 ...
 **/
ctr_object *
ctr_reflect_run_for_object_ctx (ctr_object * myself,
				ctr_argument * argumentList)
{
  ctr_object *blk, *me, *argl;
  CTR_ENSURE_TYPE_BLOCK ((blk = argumentList->object));
 ...
 */
ctr_object *
ctr_reflect_register_instrumentor (ctr_object * myself,
				   ctr_argument * argumentList)
{
 ...
 */
ctr_object *
ctr_reflect_get_instrumentor (ctr_object * myself,
			      ctr_argument * argumentList)
{
  if (!ctr_instrumentor_func)
...
ctr_reflect_run_glob (ctr_object * myself, ctr_argument * argumentList)
{
  ...
  ctr_context_id = 0;
  ctr_object *ret =
    ctr_block_run_here (argumentList->object, argumentList->next,
			argumentList->object);
...
ctr_reflect_cons_str (ctr_object * myself, ctr_argument * argumentList)
{
  return
    ctr_internal_cast2string (ctr_internal_object_find_property
			      (myself,
			       ctr_build_string_from_cstring ("value"), 0));
}
