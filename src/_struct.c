...
#define IS_WRAPPED(x) \
    ((((wrapped_ffi_type*)(x))->extension_data & WRAPPED_FFI_TYPE_MAGIC) == WRAPPED_FFI_TYPE_MAGIC)
#define WRAP_DATA(x) \
...
void ctr_struct_initialize_internal()
{
    if (likely(initd))
        return;
    initd = 1;
    wrapped_ffi_type_void = (wrapped_ffi_type) { .size = ffi_type_void.size,
...
int ctr_ffi_type_struct_sizeof(
    char* format)
{ // XXX: does not generate padding bytes
    ctr_struct_initialize_internal();
    int size = 0;
...
ctr_create_ffi_type_descriptor_(char* format, int member_count, int union_)
{
    ...
    wrapped_ffi_type** elems = ctr_heap_allocate(
        sizeof(wrapped_ffi_type*) * (member_count + 1)); // plus one for the terminating NULL
    size_t current_offset = 0;
    ssize_t this_size = 0;
    size_t alignment = 0;
    char* fff = format;
