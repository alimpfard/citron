...
ctr_object* ctr_object_swap(ctr_object* myself, ctr_argument* argumentList)
{
    ...
    // LONG AF
    if (myself == CtrStdObject || myself == CtrStdBlock || myself == CtrStdString || myself == CtrStdNumber || myself == CtrStdBool || myself == CtrStdConsole || myself == CtrStdNil || myself == CtrStdGC || myself == CtrStdMap || myself == CtrStdArray || myself == CtrStdFile || myself == CtrStdSystem || myself == CtrStdDice || myself == CtrStdCommand || myself == CtrStdSlurp || myself == CtrStdShell || myself == CtrStdClock || myself == CtrStdFlow || myself == CtrStdBreak || myself == CtrStdContinue || myself == CtrStdExit || myself == CtrStdReflect || myself == CtrStdFiber || argumentList->object == CtrStdObject || argumentList->object == CtrStdBlock || argumentList->object == CtrStdString || argumentList->object == CtrStdNumber || argumentList->object == CtrStdBool || argumentList->object == CtrStdConsole || argumentList->object == CtrStdNil || argumentList->object == CtrStdGC || argumentList->object == CtrStdMap || argumentList->object == CtrStdArray || argumentList->object == CtrStdFile || argumentList->object == CtrStdSystem || argumentList->object == CtrStdDice || argumentList->object == CtrStdCommand || argumentList->object == CtrStdSlurp || argumentList->object == CtrStdShell || argumentList->object == CtrStdClock || argumentList->object == CtrStdFlow || argumentList->object == CtrStdBreak || argumentList->object == CtrStdContinue || argumentList->object == CtrStdExit || argumentList->object == CtrStdReflect || argumentList->object == CtrStdFiber) {
...
ctr_object* ctr_bool_eq(ctr_object* myself, ctr_argument* argumentList)
{
    return ctr_build_bool(
        ctr_internal_cast2bool(argumentList->object)->value.bvalue == myself->value.bvalue);
}
...
ctr_object* ctr_number_shr(ctr_object* myself, ctr_argument* argumentList)
{
    return ctr_build_number_from_float(
        (((unsigned long long)(myself->value.nvalue)) >> (unsigned long long)((ctr_internal_cast2number(argumentList->object))
                                                                                  ->value.nvalue)));
}
...
{
    return ctr_build_number_from_float(
        (((unsigned long long)(myself->value.nvalue)) | (unsigned long long)((ctr_internal_cast2number(argumentList->object))->value.nvalue)));
}
...
static const char ascii_escape_names[] = (char[256]) {
    [0x07] = 'a', [0x08] = 'b', [0x0c] = 'f', [0x0a] = 'n', [0x0d] = 'r', [0x09] = 't', [0x5c] = '\\', [0x27] = '\'', [0x22] = '"', [0x3f] = '?'
};
/**
 * [String] escape: '\n'.
 *
...
ctr_object* ctr_string_escape_ascii(ctr_object* myself,
    ctr_argument* argumentList)
{
    ctr_object* escape = argumentList && argumentList->object && argumentList->object != CtrStdNil
        ? ctr_internal_cast2string(argumentList->object)
        : ctr_build_string_from_cstring((char[]) {
            0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08,
            0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10,
            0x11, 0x12, 0x13, 0x14, 0x15, 0x16, 0x17, 0x18,
            0x19, 0x1a, 0x1b, 0x1c, 0x1d, 0x1e, 0x0a }); // sensible default
...
{ // pattern, blk, options: {!=Ignore, n=newline, i=CI}
    ...
    while ((regex_count > 0 || first) && !flagIgnore) {
        if (first)
            first = 0;
        regex_count = pcre_exec(pattern, NULL, haystack + offset,
...
ctr_object* ctr_string_reg_replace(ctr_object* myself,
    ctr_argument* argumentList)
{
    CtrStdFlow = ctr_build_string_from_cstring("Regex replace not implemented for POSIX; "
...
ctr_object* ctr_string_reg_compile(ctr_object* myself,
    ctr_argument* argumentList)
{
    CtrStdFlow = ctr_build_string_from_cstring("Regex compile not implemented for POSIX; "
                                               "use String#findPattern:do:options:");
    return myself;
}
#else
    ...
    ctr_argument* argumentList)
{
    ...
    if (argumentList->next->object) { // pattern, substitute, [options]
        ...
        ctr_object* opts = (argumentList->next->next && argumentList->next->next->object)
    ...
    ctr_argument* argumentList)
{
    ...
    char* haystack = ctr_heap_allocate_cstring(myself);
    ctr_object* answer;
    pattern = pcre_compile(needle, PCRE_EXTENDED, (char const**)&error_message,
        &regex_error, NULL);
    if (!pattern) {
        CtrStdFlow = ctr_build_string_from_cstring(error_message);
        answer = CtrStdNil;
...
ctr_object* ctr_build_listcomp(ctr_tnode* node)
{
    ...
    if (generators) {
        ...
        while (generator) {
            ...
            if (generator->node->type == CTR_AST_NODE_NESTED && generator->node->nodes->node->type == CTR_AST_NODE_EXPRMESSAGE && generator->node->nodes->node->nodes->node->vlen == 2 && strncmp(generator->node->nodes->node->nodes->node->value, "me", generator->node->nodes->node->nodes->node->vlen) == 0) {
                ...
                if (generator->node->type == CTR_AST_NODE_KWMESSAGE) {
                    ...
                    while ((sv = strchr(generator->node->value, ':')) != NULL && i++ == 0)
                        ;
                    if (i != 2) {
                        CtrStdFlow = ctr_format_str("EExpected exactly one keyword to name the "
    ...
    ctr_object* gen_handler_s = ctr_build_string_from_cstring("{:obj "
    ...
    if (generators && !preds) { // no filter: [e ,, gs@g+] -> [evaluate(e) ]
        ctr_object* filter_s = ctr_build_string_from_cstring("{:gen "
                                                             "var syms is my syms. "
                                                             "^{:blk "
        ...
        ctr_object* filter_sv = ctr_build_string_from_cstring("{"
                                                              ...
                                                              "^(my names fmap: \\:__vname gen-handler "
                                                              "applyTo: (Reflect getObject: __vname)) "
    ...
    ctr_object* filter_sv = ctr_build_string_from_cstring(
        "{"
        "var gen-handler is my gen-handler."
        "var nvs is (my names fmap: \\:__vname gen-handler applyTo: (Reflect "
        "getObject: __vname))."
    ...
    ctr_internal_object_add_property(filter_svobj,
        ctr_build_string_from_cstring("gen-handler"),
        gen_handler, CTR_CATEGORY_PRIVATE_PROPERTY);
    ctr_internal_object_add_property(
        filter_svobj, ctr_build_string_from_cstring("names"), resolved_refs,
        CTR_CATEGORY_PRIVATE_PROPERTY);
    ctr_internal_object_add_property(filter_svobj,
        ...
        if (!next) {
            overload->sub_buckets = overload->bucket_count++
                ? ctr_heap_reallocate(overload->sub_buckets,
                    sizeof(ctr_overload_set*) * overload->bucket_count)
    ...
    ctr_object* my)
{
    ...
    if (likely(parameterList && parameterList->node)) {
        ...
        while (argList) {
            ...
            if (unlikely(parameter->type == CTR_AST_NODE_NESTED)) {
                ...
                ctr_send_message(
                    ...
                    &(ctr_argument) {
                        pattern, &(ctr_argument) { ctr_contexts[ctr_context_id], NULL } }); // hand the block the context
            } else {
                was_vararg = (strncmp(parameter->value, "*", 1) == 0);
                if (!argList->object) {
                    ctr_object* arr = was_vararg ? ctr_array_new(CtrStdArray, NULL) : CtrStdNil;
        ...
        while (parameterList) {
            ...
            ctr_object* name = ctr_build_string(parameterList->node->value + was_vararg,
...
{
    return object == CtrStdWorld || object == CtrStdObject || object == CtrStdBlock || object == CtrStdString || object == CtrStdNumber || object == CtrStdBool || object == CtrStdConsole || object == CtrStdNil || object == CtrStdGC || object == CtrStdMap || object == CtrStdArray || object == CtrStdIter || object == CtrStdFile || object == CtrStdSystem || object == CtrStdDice || object == CtrStdCommand || object == CtrStdSlurp || object == CtrStdShell || object == CtrStdClock || object == CtrStdReflect || object == CtrStdReflect_cons || object == CtrStdFiber || object == CtrStdThread || object == CtrStdSymbol;
}
...
extern int
    more_exception_data; /* flag: should we display more data for exceptions? */
        ...
        while (mapItem) {
            if (line > -1 && mapItem->node->type == CTR_AST_NODE_PROGRAM) {
                ...
                printf(" (%s: %d)%s", currentProgram, line + 1,
                    ignored && more_exception_data ? CTR_ANSI_COLOR_CYAN
                        " [Ignored]" CTR_ANSI_COLOR_RESET
                                                   : "");
                break;
            }
            if (line == -1 && mapItem->node == stackNode) {
