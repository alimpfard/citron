...
ctr_object* ctr_array_view(ctr_object* myself, ctr_argument* argumentList)
{
    ...
    int length = endo ? endo->value.nvalue
 ...
 *
 * Repeats the array o times if o is a number, and generates an array
 * multiplication for myself and o if o is an array
 */
    ...
    ctr_argument* argumentList)
{
    ...
    for (ctr_size
             s
         = ceil((myself->value.avalue->head - myself->value.avalue->tail) / (double)icount),
         i = 0;
         i < s; i++) {
...
ctr_object* ctr_array_get(ctr_object* myself, ctr_argument* argumentList)
{
    ...
    if (getIndex->info.type != CTR_OBJECT_TYPE_OTNUMBER) {
        ...
        char *typename_ = ctr_heap_allocate_cstring(ctr_internal_cast2string(
                 ctr_send_message(getIndex, "type", 4, NULL))),
    ...
    for (int i = 0;; i++) {
        ...
        for (int x = myself->value.avalue->tail; x < myself->value.avalue->head;
            ...
            if (element->info.type != CTR_OBJECT_TYPE_OTARRAY)
                ctr_array_push(tarr, &(ctr_argument) { element, NULL });
...
ctr_object* ctr_array_last(ctr_object* myself, ctr_argument* argumentList)
{
    return ctr_send_message_variadic(
        myself, "at:", 3, 1,
        ctr_build_number_from_float(myself->value.avalue->head - myself->value.avalue->tail - 1));
}
 ...
 * Array imap: [Block]
 *
 * Maps a function over the block. this function should accept an index and a
 * value.
 *
 ...
 *
 * Maps a function over the block. this function should accept an index and a
 * value. Modifies the collection inplace
 *
 ...
 *
 * reduces an array according to a block (which takes an accumulator and the
 * value, and returns the next acc) from the left (index 0)
 *
 * ([1,2,3,4]) foldl: {:acc:v ^acc + v.} accumulator: 0. #=> Equivalent to
 * ([1,2,3,4]) sum.
 */
 ...
 *
 * reduces an array according to a block (which takes an accumulator and the
 * value, and returns the next acc) from the left (index 0) Errors if there's no
...
/**
 * [Array] fmap: [Block] from: [Block|Array|Map|supports `items`] [filter:
 * [Block]]
 * @Experimental
 * list comprehension
 */
    ...
    ctr_argument* argumentList)
{
    ctr_object *selector = argumentList->object,
               ...
               *ifexp = argumentList->next->next && argumentList->next->next->object
        ? argumentList->next->next->object
        : NULL,
               *result = ctr_array_new(CtrStdArray, NULL), *elem, *old_from;
    ...
    ctr_argument* argumentList)
{
    ...
    if (myself->value.avalue->tail == myself->value.avalue->head) {
        newArgumentList->object = ctr_build_string_from_cstring(
            myself->value.avalue->immutable ? "[" : CTR_DICT_CODEGEN_ARRAY_NEW);
    ...
    for (i = myself->value.avalue->tail; i < myself->value.avalue->head; i++) {
        ...
        if (!arrayElement) {
            ...
            string = ctr_string_append(string, newArgumentList);
        } else if (arrayElement->info.type == CTR_OBJECT_TYPE_OTBOOL || arrayElement->info.type == CTR_OBJECT_TYPE_OTNUMBER || arrayElement->info.type == CTR_OBJECT_TYPE_OTNIL) {
    ...
    if (argumentList->object) {
        ...
        default:
            CtrStdFlow = ctr_format_str(
                "EMap::'new:' expects a block, not %s",
                ctr_send_message(argumentList->object, "type", 4, NULL));
        }
    }
...
ctr_object* ctr_map_put(ctr_object* myself, ctr_argument* argumentList)
{
    ...
    if (!hasher) {
        ...
        /* If developer returns something other than string (ouch, toString), then
         * cast anyway */
        ...
        /* If developer returns something other than a number (ouch, iHash), then
         * hash that */
        ctr_number hashk;
 ...
 *
 * Iterates over the map, passing the key and the value to the function, and
 * replacing the value with the result (Or itself if a value is not returned)
 *
 */
 ...
 * [Map] kvmap: [Block]
 *
 * Iterates over the map, passing a tuple of [key, value] to the specified
 * block. And constructs a new Map based off the returned kv-tuple Note that
 * within an each/map block, 'me' and 'my' refer to the collection.
 */
 ...
 *
 * Iterates over the map, passing a tuple of [key, value] to the specified
 * block. And constructs a new list based off the returned kv-tuple Note that
 * within an each/map block, 'me' and 'my' refer to the collection.
 */
ctr_object* ctr_map_kvlist(ctr_object* myself, ctr_argument* argumentList)
{
    ...
    while (m) {
        ...
        /* if (arguments->object->info.type != CTR_OBJECT_TYPE_OTARRAY &&
           ...
           {
           CtrStdFlow = ctr_build_string_from_cstring ("kvmap block must only return
           a 2-tuple or nothing."); return CtrStdNil;
           } */
        if (arguments->object == block) {
            ctr_object* lst = ctr_array_new(CtrStdArray, NULL);
            arguments->object = m->key;
 ...
 * [Map] flip
 *
 * flips the keys and the values of the map. (same-value keys will be
 * overwritten)
 */
 ...
 * accidentally dumping the entire contents can be annoying, in this case you
 * can override the toString behaviour but you don't have to remap the original,
 * you can just use this alias. Also, this alias can be used if you want to make
...
ctr_object* ctr_map_to_string(ctr_object* myself, ctr_argument* argumentList)
{
    ...
    while (mapItem) {
        ...
        if (mapItem->value == myself) {
            newArgumentList->object = ctr_build_string_from_cstring("':selfReference:'");
            ctr_string_append(string, newArgumentList);
        } else if (mapItem->value->info.type == CTR_OBJECT_TYPE_OTBOOL || mapItem->value->info.type == CTR_OBJECT_TYPE_OTNUMBER || mapItem->value->info.type == CTR_OBJECT_TYPE_OTNIL) {
        ...
        ctr_string_append(string, newArgumentList);
        if (mapItem->key->info.type == CTR_OBJECT_TYPE_OTBOOL || mapItem->key->info.type == CTR_OBJECT_TYPE_OTNUMBER || mapItem->value->info.type == CTR_OBJECT_TYPE_OTNIL) {
 ...
 *
 * e.g. (Map new put: 'Hello' at: 'test0', put: 'World' at: 'test1') unpack:
 * (Map cnew: {my test0 is 'a'. my test1 is 'b'.})
 *
 * (Always prefer using algebraic deconstruction assignments: look at section
 * 'Assignment')
 */
...
ctr_object* ctr_map_assign(ctr_object* myself, ctr_argument* argumentList)
{
    ...
    while (mapItem) {
        ...
        if (!ctr_reflect_check_bind_valid(mapItem->value, value, 0)) {
            // // CtrStdFlow = NULL; //get rid of the error, and bind the result back
            // to the name newArgumentList->object = mapItem->key; ctr_send_message
            // (mapItem->value, "unpack:", 7, newArgumentList); value->info.raw = 0;
            CtrStdFlow = ctr_build_string_from_cstring("Invalid bind");
            break;
        } else {
...
uint32_t ctr_internal_string_hash(ctr_object* key)
{
    ...
    return siphash24(key->value.svalue->value, key->value.svalue->vlen,
        CtrHashKey);
}
KHASH_INIT(ctr_hmap_t, ctr_object*, ctr_object*, 1, ctr_internal_string_hash,
...
ctr_object* ctr_hmap_each(ctr_object* myself, ctr_argument* argumentList)
{
    ...
    for (khiter_t k = kh_begin(h); k != kh_end(map); ++k) {
        if (kh_exist(map, k)) {
            ctr_block_run(blk,
                &(ctr_argument) { kh_key(map, k),
                    &(ctr_argument) { kh_value(map, k), NULL } },
                NULL);
        }
        if (CtrStdFlow) {
            if (CtrStdFlow == CtrStdContinue) {
