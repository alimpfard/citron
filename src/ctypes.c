 ...
 */
wrapped_ffi_type* ctr_ctype_citron_object_try_infer_type(ctr_object* object)
{
...
{ // OUCH, too big for literal citron
    ...
    ctr_argument* argumentList)
{
    ...
    if (myself->info.type != CTR_OBJECT_TYPE_OTEX) {
        ...
        ret->value.rvalue->ptr = (meta == CtrStdCType_struct)
...
wrapped_ffi_type* ctr_ctypes_ffi_convert_ctype_to_ffi_type(ctr_ctype type)
{
    switch (type) {
    case CTR_CTYPE_VOID:
        return &wrapped_ffi_type_void;
...
error_out_inf:;
    ...
    if (error == 5) {
        ...
        size_t slen = sprintf(ERROR_STR, "Type inferrence failed for %s (argument#%d)",
...
{ //<cif, CTypes pointer (fn), Array avalues; ^Citron object
    ...
    for (int i = 0; i < asize; i++) {
        ...
        else if ((type == (ffi_type*)&wrapped_ffi_type_pointer || type == &ffi_type_pointer) && obj->info.type == CTR_OBJECT_TYPE_OTBLOCK) {
...
{
    if (argumentList->object->info.type == CTR_OBJECT_TYPE_OTEX && (argumentList->object->value.rvalue->type == CTR_CTYPE_POINTER || argumentList->object->value.rvalue->type == CTR_CTYPE_STRING)) {
        myself->value.rvalue->ptr = argumentList->object->value.rvalue->ptr;
