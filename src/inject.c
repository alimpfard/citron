...
ctr_inferred_ctype_type_t ctr_inject_type_to_ctype(Sym* svv, CType* type)
{
    ...
    case VT_ENUM:
        return (ctr_inferred_ctype_type_t) { .const_value = ctr_build_number_from_float(svv->c),
            .kind = KIND_CONSTANT };
    case VT_STRUCT:
        /* TODO: fix structs */
        // return (ctr_inferred_ctype_type_t){.vtype=NULL};
        ...
        while ((s = s->next) != NULL) {
            ...
            if (!pad) {
                ...
                // printf("FIELD %.*s\n", token->len, token->str);
                // todo get field name
                ctr_array_push(descriptor.names, &(ctr_argument) { ctr_build_string(token->str, token->len) });
            }
    ...
    ctr_argument* argumentList)
{
    ...
    s = symbols;
    typeof(ctr_map_contains)* fn_all_contains_val = !all ? &ctr_true
    ...
    ctr_argument* argumentList)
{
    ...
    if (!r || !(ds = r->ptr) || ds->info.relocated) {
        CtrStdFlow = ctr_build_string_from_cstring("Inject object is in an invalid state "
    ...
    ctr_argument* argumentList)
{
    ...
    if (sym->info.type != CTR_OBJECT_TYPE_OTSTRING) {
        CtrStdFlow = ctr_format_str("EExpected String but got %S",
