...
public:
    ...
    {
        if (instance == nullptr)
            return instance = std::make_shared<NativeLlvmTarget>();
        return instance;
    }
    NativeLlvmTarget()
    {
...
public:
    KaleidoscopeJIT()
        ...
        , TM(EngineBuilder().setErrorStr(&errorStr).selectTarget()
                  ?: (puts(errorStr.c_str()), nullptr))
...
void llvmDebugPrint(BasicBlock* value) { errs() << *value; }
void llvmDebugPrint(Value* value) { errs() << *value; }
void llvmDebugPrint(Type* value) { errs() << *value; }
void llvmDebugPrint(Module* value) { errs() << *value; }
...
public:
    GenContext() { module = new Module("citron-inline", _ctx); }
...
Value* generateInlineAsm(std::string asmdec, std::string constraint,
    InlineAsm::AsmDialect dialect, FunctionType* AsmFTy)
{
    auto* value = InlineAsm::get(AsmFTy, asmdec, StringRef(constraint), true,
        false, dialect);
    return value;
}
...
std::string generateEntryFunction(std::string asmdec, std::string constr,
    InlineAsm::AsmDialect dialect, int off,
    int cnt, asm_arg_info_t* arginfo,
    GenContext& context)
{
    ...
    FunctionType* FTy = FunctionType::get(
        PointerType::get(Type::getInt8Ty(_ctx), 0), FnArgTypes, false);
    std::string name = "__entry_inline_" + std::to_string(cid++);
    Function* func = Function::Create(FTy, GlobalValue::ExternalLinkage,
        (name).c_str(), context.module);
    ...
    if (cnt > 0) {
        ...
        for (int i = 0; i < cnt; i++) {
            ...
            auto* arg = args;
            if (addr_a > 0)
                arg = GetElementPtrInst::CreateInBounds(
                    arg, ConstantInt::get(i8ty, addr_a, false), "", blk);
            arg = new BitCastInst(arg, i8ppty, "", blk);
            arg = new LoadInst(i8ppty, arg, "", blk);
            arg = GetElementPtrInst::CreateInBounds(
                arg, ConstantInt::get(i8ty, off, false), "", blk);
            ...
            if (dty == ASM_ARG_TY_INT)
                ...
                arg = new IntToPtrInst(new FPToSIInst(arg, i64ty, "", blk), i8pty, "",
                    blk);
                argty = i8pty;
            }
            if (i != cnt - 1) {
                args = GetElementPtrInst::CreateInBounds(
                    args, ConstantInt::get(i8ty, addr_b, false), "", blk);
    ...
    retval = CallInst::Create(
        context.module->getFunction("ctr_build_number_from_float"),
        std::vector<Value*> { retval }, "", blk);
...
void* GenContext::run(std::string dec, std::string ct,
    InlineAsm::AsmDialect dialect, int offset, int count,
    asm_arg_info_t* arginfo)
{
    std::vector<Type*> srtsArgs { Type::getDoubleTy(_ctx) };
    FunctionType* malloc_ty = FunctionType::get(Type::getInt8PtrTy(_ctx), srtsArgs, false);
    Function* ctr_build_number_from_float_f = Function::Create(malloc_ty, Function::ExternalLinkage,
        Twine("ctr_build_number_from_float"), module);
...
extern "C" void* ctr_cparse_intern_asm_block(char* asm_begin, char* asm_end,
    char* constraints, int offset,
    int argc, asm_arg_info_t* arginfo,
    int att)
{
    ...
    if ((fn = context.run(s, constraints,
             att ? InlineAsm::AsmDialect::AD_ATT
                 : InlineAsm::AsmDialect::AD_Intel,
             offset, argc + 1, arginfo))
        == nullptr) {
        return NULL;
    }
    free(s);
