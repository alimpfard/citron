...
int ctr_transform_lambda_shorthand = 0; /* flag: indicates whether lexer has seen a shorthand lambda (\(:arg)+
                                           expr) */
...
char* ctr_clex_readfstr()
{
    ...
    while (1) {
        ...
        if (c == '\n')
            ctr_clex_line_number++;
        if (c == -30) {
            if (((ctr_code + 1) < ctr_eofcode) && *(ctr_code + 1) == -128 && ((ctr_code + 2) < ctr_eofcode) && *(ctr_code + 2) == -70) {
                break;
            }
        }
        ctr_clex_tokvlen++;
...
char* ctr_clex_readstr()
{
    ...
    while (/* reading string in non-verbatim mode, read until the first
        ...
        if (!ctr_clex_verbatim_mode && !escape && c == '$') {
            ...
            if (((ctr_code + 1) < ctr_eofcode) && *(ctr_code + 1) == '{' && ((ctr_code + 2) < ctr_eofcode) && *(ctr_code + 2) == '{') {
