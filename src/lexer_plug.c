...
static int ctr_lex_verbatim_mode = 0;                    /* flag: indicates whether lexer operates in verbatim mode or not (1 =
                                                            ON, 0 = OFF) */
...
int ctr_lex_tok()
{
    ...
    if (((c == 'i') && (ctr_code + 1) < ctr_eofcode && (*(ctr_code + 1) == 's') && isspace(*(ctr_code + 2))) || ((c == ':') && (ctr_code + 1) < ctr_eofcode && (*(ctr_code + 1) == '='))) {
        ctr_code += 2;
    ...
    //   return CTR_TOKEN_REF;
    // }
    while (!isspace(c) && (c != '#' && c != '(' && c != ')' && c != '[' && c != ']' && c != '{' && c != '}' && c != '.' && c != ',' && c != '^' && (!((ctr_code + 2) < ctr_eofcode && (uint8_t)c == 226 && ((uint8_t) * (ctr_code + 1) == 134) && ((uint8_t) * (ctr_code + 2) == 145))) && (c != ':') && c != '\'') && ctr_code != ctr_eofcode) {
        ctr_lex_buffer[i] = c;
        ctr_lex_tokvlen++;
        i++;
        if (i > ctr_lex_bflmt) {
...
char* ctr_lex_readstr()
{
    ...
    while ((/* reading string in non-verbatim mode, read until the first
        ...
        || (/* reading string in verbatim mode, read until the '<?' sequence */
            ctr_lex_verbatim_mode == 1 && !(c == '<' && ((ctr_code + 1) < ctr_eofcode) && *(ctr_code + 1) == '?') && (ctr_code < ctr_eofcode))) {
