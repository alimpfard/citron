...
// FORMAT <type> [<size/length>] [m <length> <metadata_methods>] [p <length>
// <metadata_properties>] [<value(s)>]
...
void ctr_marshal_keyvalues(ctr_mapitem *, ctr_size, char **, ctr_size *,
                           ctr_size *);
ctr_object *ctr_unmarshal_object(char *stream, ctr_size *avail,
                                 ctr_size *consumed);
...
ctr_tnode *ctr_unmarshal_ast(char *stream, ctr_size avail, ctr_size *consumed) {
  ...
  case CTR_MARSH_AST_KWMESSAGE: { // KWMESSAGE <msg_len> <message>
...
void ctr_marshal_ast(ctr_tnode *rnode, char **stream, ctr_size *stream_len,
                     ctr_size *used) {
  ...
  case CTR_AST_NODE_EXPRMESSAGE: { // EXPRMESSAGE <receiver> <msg_node_count>
                                   // (<message>)*
  ...
  case CTR_AST_NODE_BINMESSAGE: { // BINMESSAGE <msg_len(in case of ::)> <msg>
                                  // <argument> (count = 1 implicit)
  ...
  case CTR_AST_NODE_KWMESSAGE: { // KWMESSAGE <msg_len> <message>
                                 // <argument_count> (<argument>)+
  ...
  case CTR_AST_NODE_CODEBLOCK: { // CODEBLOCK (<argument>)* (<instruction>)*
    memset(*stream + ((*used)++),
           rnode->lexical ? CTR_MARSH_AST_CODEBLOCKL
                          : CTR_MARSH_AST_CODEBLOCKNL,
           1);
    // *Params
...
void ctr_marshal_meta_object(ctr_object *obj, char **stream,
                             ctr_size *stream_len,
                             ctr_size *used) { // don't pass bullshit
...
void ctr_marshal_keyvalues(ctr_mapitem *head, ctr_size hlen, char **stream,
                           ctr_size *stream_len, ctr_size *used) {
  if (!*stream) {
  ...
  *used += sizeof(ctr_size);
  ...
  for (; hlen > 0; hlen--) {
    ...
    if (head->value->info.type == CTR_OBJECT_TYPE_OTNATFUNC)
      continue;
...
void ctr_marshal_store_link(ctr_object *obj, char **stream,
                            ctr_size *stream_len, ctr_size *used) {
  if (obj == CtrStdObject)
...
ctr_object *ctr_marshal_resolve_linq(char *stream, ctr_size *stream_len,
                                     ctr_size *used) {
  char type = *(stream + ((*used)++));
  switch (type) {
  case CTR_MARSH_OBJECT_LINK_Object:
    return CtrStdObject;
  case CTR_MARSH_OBJECT_LINK_Block:
    return CtrStdBlock;
  case CTR_MARSH_OBJECT_LINK_String:
    return CtrStdString;
  case CTR_MARSH_OBJECT_LINK_Number:
    return CtrStdNumber;
  case CTR_MARSH_OBJECT_LINK_Bool:
    return CtrStdBool;
  case CTR_MARSH_OBJECT_LINK_Console:
    return CtrStdConsole;
  case CTR_MARSH_OBJECT_LINK_Nil:
    return CtrStdNil;
  case CTR_MARSH_OBJECT_LINK_GC:
    return CtrStdGC;
  case CTR_MARSH_OBJECT_LINK_Map:
    return CtrStdMap;
  case CTR_MARSH_OBJECT_LINK_Array:
    return CtrStdArray;
  case CTR_MARSH_OBJECT_LINK_Iter:
    return CtrStdIter;
  case CTR_MARSH_OBJECT_LINK_File:
    return CtrStdFile;
  case CTR_MARSH_OBJECT_LINK_System:
    return CtrStdSystem;
  case CTR_MARSH_OBJECT_LINK_Dice:
    return CtrStdDice;
  case CTR_MARSH_OBJECT_LINK_Command:
    return CtrStdCommand;
  case CTR_MARSH_OBJECT_LINK_Slurp:
    return CtrStdSlurp;
  case CTR_MARSH_OBJECT_LINK_Shell:
    return CtrStdShell;
  case CTR_MARSH_OBJECT_LINK_Clock:
    return CtrStdClock;
  case CTR_MARSH_OBJECT_LINK_Reflect:
    return CtrStdReflect;
  case CTR_MARSH_OBJECT_LINK_Reflect_cons:
    return CtrStdReflect_cons;
  case CTR_MARSH_OBJECT_LINK_Fiber:
    return CtrStdFiber;
  case CTR_MARSH_OBJECT_LINK_Thread:
    return CtrStdThread;
  case CTR_MARSH_OBJECT_LINK_Symbol:
    return CtrStdSymbol;
  default:
  case CTR_MARSH_OBJECT_LINK_DESCRIBE:
    return NULL; // TODO;
  }
}

ctr_size
ctr_marshal_object(ctr_object *obj, char **stream,
                   ctr_size *stream_len /* in char* */,
                   ctr_size *used) { // We will allocate stream if it's NULL, or
                                     // use it as-is (will assume the size fits)
  if (!*stream) {
    *stream = ctr_heap_allocate(sizeof(char) * 1024);
    *stream_len = 1024;
  }
  ...
  case CTR_OBJECT_TYPE_OTSTRING: {
    ...
    memcpy(buf, obj->value.svalue->value,
           sizeof(char) * obj->value.svalue->vlen);
    break;
  }
  case CTR_OBJECT_TYPE_OTARRAY: {
    ...
    for (ctr_size ie = obj->value.avalue->tail; ie < obj->value.avalue->head;
         ie++) {
      (void)ctr_marshal_object(obj->value.avalue->elements[ie], &buf, &blen,
                               &bused);
    }
    len = bused;
    break;
...
ctr_map *ctr_unmarshal_keyvalues(char *stream, ctr_size *avail,
                                 ctr_size *consumed) {
  ...
  *consumed += sizeof(ctr_size);
  ...
  map->head = head;
  for (; len > 0; len--) {
    ctr_object *key = ctr_unmarshal_object(stream, avail, consumed),
               *value = ctr_unmarshal_object(stream, avail, consumed);
    if (!key || !value) {
      printf("NULL encountered in deserialization of keyvalue: [%lu]%p:%p "
             "(around '",
             len, key, value);
...
void ctr_unmarshal_meta_object_into(ctr_object *obj, char *stream,
                                    ctr_size *avail, ctr_size *consumed) {
  ...
  if (!obj)
    return; // read the metadata, but skip adding them
  if (meths) {
...
ctr_object *ctr_unmarshal_object(char *stream, ctr_size *avail,
  ...
  // if(umarshed&&umarshed->info.type == CTR_OBJECT_TYPE_OTOBJECT)
  // ctr_set_link_all(umarshed, ctr_marshal_resolve_linq(stream, avail,
  // consumed))
  return umarshed;
}
...
ctr_object *ctr_reflect_marshal(ctr_object *myself,
...
ctr_object *ctr_reflect_unmarshal(ctr_object *myself,
                                  ctr_argument *argumentList) {
  ctr_object *ret = ctr_unmarshal(argumentList->object->value.svalue->value,
