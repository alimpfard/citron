...
void* ctr_heap_allocate(size_t size)
{
    ...
    /* Store the width of the memory block in the slice itself so we can always
     * find it */
...
void* ctr_heap_allocate_shared(size_t size)
{
    ...
    if (slice_of_memory == MAP_FAILED) {
        printf("Out of memory. Failed to allocate %lu bytes (mmap failed). \n",
            size);
        ctr_print_stack_trace();
        abort();
        exit(1);
...
void* ctr_heap_allocate_tracked(size_t size)
{
    ...
    if (numberOfMemBlocks >= maxNumberOfMemBlocks) {
        if (memBlocks == NULL) {
            ...
            memBlocks = ctr_heap_reallocate(
                memBlocks, (sizeof(memBlock) * (maxNumberOfMemBlocks)));
        }
    }
 ...
 */
size_t ctr_heap_get_latest_tracking_id() { return numberOfMemBlocks - 1; }
