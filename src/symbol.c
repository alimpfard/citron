...
ctr_object *ctr_symbol_to_string(ctr_object *myself,
  ...
  char *name =
      ctr_heap_allocate(sizeof(char) * (myself->value.svalue->vlen + 1));
...
ctr_object *ctr_symbol_to_string_s(ctr_object *myself,
                                   ctr_argument *argumentList) {
  return ctr_get_or_create_symbol_table_entry_s(myself->value.svalue->value,
  ...
  if (ctr_reflect_get_primitive_link(argumentList->object) == CtrStdSymbol) {
    if (argumentList->object != myself) {
      CtrStdFlow = ctr_build_string_from_cstring(
          "Cannot bind symbols with different values");
    }
  }
  return myself;
}
