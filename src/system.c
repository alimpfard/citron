...
#ifdef withBoehmGC
                                           GC_get_heap_size()
        - GC_get_free_bytes() - GC_get_unmapped_bytes()
#else
            ctr_gc_alloc
#endif
    );
    ...
    ctr_argument* argumentList)
{
    ...
    command = (char*)ctr_heap_allocate(
        (sizeof(char) * len)); /* actually we need +1 for the space between commands, but we dont
                                  because we remove the colon : ! */
    strncpy(command, prefix->value.svalue->value,
        prefix->value.svalue->vlen - 1); /* remove colon, gives room for space */
