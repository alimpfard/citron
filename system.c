...
void ctr_gc_sweep(int all)
{
	...
	while (currentObject) {
		...
		if ((currentObject->info.mark == 0
		     && currentObject->info.sticky == 0) || all) {
			...
			if (currentObject->value.rvalue != NULL) {
				if (currentObject->release_hook)
					((voidptrfn_t *) currentObject->
					 release_hook) (currentObject->value.
							rvalue->ptr);
				free_heap_maybe_shared(currentObject->value.
						       rvalue);
			}
				  ...
				  ctr_argument * argumentList)
{
	...
	} else thread = ((ctr_thread_t*)myself->value.rvalue->ptr)->thread;
...
ctr_object *ctr_thread_run(ctr_object * myself, ctr_argument * argumentList)
{
	...
	pthread_mutex_unlock(((ctr_thread_t *) myself->value.rvalue->ptr)->
			     mutex);
	return myself;
}
