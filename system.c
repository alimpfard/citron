...
void ctr_gc_mark(ctr_object * object, int last_vector_index)
{
	...
	if (last_vector_index > -1)
		for (int i = 0;
		     i < CTR_GC_BACKLOG_MAX && i <= last_vector_index; i++)
...
void ctr_gc_sweep(int all)
{
	...
	while (currentObject) {
		...
		if (all
		    || ((currentObject->info.mark | currentObject->info.sticky)
			== 0)) {
			...
			case CTR_OBJECT_TYPE_OTSTRING:
				if (currentObject->value.svalue != NULL) {
					if (currentObject->value.svalue->vlen >
					    0) {
						free_heap_maybe_shared
						    (currentObject->
						     value.svalue->value);
					}
					free_heap_maybe_shared
					    (currentObject->value.svalue);
				}
				break;
			case CTR_OBJECT_TYPE_OTARRAY:
				free_heap_maybe_shared(currentObject->
						       value.avalue->elements);
				free_heap_maybe_shared(currentObject->
						       value.avalue);
				break;
			case CTR_OBJECT_TYPE_OTEX:
				if (currentObject->value.rvalue != NULL) {
					if (currentObject->release_hook)
						((voidptrfn_t *)
						 currentObject->release_hook)
						    (currentObject->value.
						     rvalue->ptr);
					free_heap_maybe_shared
					    (currentObject->value.rvalue);
				}
				break;
			}
			free_heap_maybe_shared(currentObject);
...
ctr_object *ctr_gc_sweep_this(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if(myself->lexical_name) ctr_internal_object_delete_property(ctr_contexts[ctr_context_id], myself->lexical_name, 0);
	return CtrStdNil;
}
...
{
	ctr_gc_memlimit =
	    (uint64_t) ctr_internal_cast2number(argumentList->object)->
	    value.nvalue;
	return myself;
}
				     ...
				     ctr_argument * argumentList)
{
	...
	if ((str->value.svalue->vlen > 0)
		...
		memcpy(ncstr, str->value.svalue->value,
...
void ctr_check_permission(uint8_t operationID)
{
	...
	if ((ctr_command_security_profile & operationID)) {
		reason =
		    "This program is not allowed to perform this operation.";
		if (operationID == CTR_SECPRO_NO_SHELL) {
			reason =
			    "This program is not allowed to execute shell commands.";
		}
...
ctr_object *ctr_command_accept(ctr_object * myself, ctr_argument * argumentList)
{
	...
	while (1
		...
		if (stringObj->value.svalue->vlen >= 6 && strncmp(stringObj->value.svalue->value, "Array ", 6) == 0) {	//it was a correct message
			messageDescriptorArray =
			    ctr_string_eval(stringObj, NULL);
			if (messageDescriptorArray->info.type ==
			    CTR_OBJECT_TYPE_OTARRAY) {
				messageSelector =
				    ctr_array_shift(messageDescriptorArray,
						    NULL);
			} else {
				messageSelector =
				    ctr_internal_cast2string
			       ...
			       char part)
{
	...
	case 'Y':
		answer =
		    ctr_build_number_from_float((ctr_number) date->tm_year +
						1900);
		break;
	case 'm':
		answer =
		    ctr_build_number_from_float((ctr_number) date->tm_mon + 1);
		break;
	case 'd':
		answer =
		    ctr_build_number_from_float((ctr_number) date->tm_mday);
		break;
	case 'H':
		answer =
		    ctr_build_number_from_float((ctr_number) date->tm_hour);
		break;
	case 'i':
...
ctr_object *ctr_clock_week(ctr_object * myself, ctr_argument * argumentList)
{
	...
	weekNumber =
	    ctr_internal_cast2number(ctr_build_string_from_cstring(str));
	ctr_heap_free(str);
	return weekNumber;
}
...
void ctr_clock_init(ctr_object * clock)
{
	...
	ctr_internal_object_add_property(clock,
					 ctr_build_string_from_cstring
					 (CTR_DICT_ZONE),
					 ctr_build_string_from_cstring("UTC"),
					 0);
}
				...
				ctr_argument * argumentList)
{
	...
	long int init = clock();
	ctr_block_run_here(argumentList->object, args,
				  ...
				  ctr_argument * argumentList)
{
	...
	if (!myself->value.rvalue->ptr) {
		...
		pthread_mutex_t *mutex =
		    ctr_heap_allocate(sizeof(pthread_mutex_t));
	...
	((ctr_thread_t *) myself->value.rvalue->ptr)->target =
	    argumentList->object;
	...
	sprintf(name, "%.*s-%.*d", 15 - sizeof(ctr_size), pname,
		sizeof(ctr_size), thread_current_number % (sizeof(ctr_size)));
				...
				ctr_argument * argumentList)
{
	...
	((ctr_thread_t *) myself->value.rvalue->ptr)->args =
	    argumentList->object;
	pthread_mutex_lock(((ctr_thread_t *) myself->value.rvalue->ptr)->mutex);
	pthread_create(thread, NULL, ctr_run_thread_func,
		       myself->value.rvalue->ptr);
				       ...
				       ctr_argument * argumentList)
{
	...
	((ctr_thread_t *) inst->value.rvalue->ptr)->target =
	    argumentList->object;
...
ctr_object *ctr_thread_run(ctr_object * myself, ctr_argument * argumentList)
{
	...
	pthread_mutex_unlock(((ctr_thread_t *) myself->value.rvalue->
			      ptr)->mutex);
	return myself;
}
...
ctr_object *ctr_thread_join(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if (pthread_join
	    (*(((ctr_thread_t *) myself->value.rvalue->ptr)->thread),
	     &retval) != 0) {
		CtrStdFlow =
		    ctr_build_string_from_cstring("Thread could not join");
		return CtrStdNil;
	}
...
ctr_object *ctr_thread_name(ctr_object * myself, ctr_argument * argumentList)
{
	...
	if ((err = pthread_getname_np(*threadt, name, 16)) != 0) {
		char errbuf[1024];
		sprintf(errbuf, "Error when reading thread name: %s",
			strerror(err));
...
ctr_object *ctr_thread_names(ctr_object * myself, ctr_argument * argumentList)
{
	...
	ctr_object *str;
	if ((str =
	     ctr_internal_cast2string(argumentList->object))->value.
	    svalue->vlen > 15) {
		CtrStdFlow =
		    ctr_build_string_from_cstring
	...
	if ((err = pthread_setname_np(*threadt, name)) != 0) {
		char errbuf[1024];
		sprintf(errbuf, "Error when setting thread name: %s",
			strerror(err));
		CtrStdFlow = ctr_build_string_from_cstring(errbuf);
		return CtrStdNil;
	}
