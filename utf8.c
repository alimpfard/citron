...
*/
ctr_size
getBytesUtf8 (char *strval, long startByte, ctr_size lenUChar)
{
  long i = 0;
  long bytes = 0;
  int s = 0;
 ...
 */
ctr_size
ctr_getutf8len (char *strval, ctr_size max)
{
  ctr_size i;
  ctr_size j = 0;
  ctr_size s = 0;
...
ctr_size
ctr_getutf8clustercount (char *strval, ctr_size max)
{
...
ctr_getutf8clustercount (char *strval, ctr_size max)
{
  ...
  int count = 0;
  // printf("%.*s\n", max, strval);
  for (int end = ubrk_next (BIter__); end != UBRK_DONE; start = end, end = ubrk_next (BIter__))
    {
      long c = 0;
...
int
ctr_utf8_is_one_cluster (char *strval, ctr_size max)
{
