...
ctr_object *ctr_cwlk_return(ctr_tnode *node) {
  ...
  li = node->nodes;
  if (!li->node) {
    printf("Invalid return expression 2 (Return statement has no parse node -- "
           "%p has no node %p).\n",
           li, li->node);
    exit(1);
  }
...
void ctr_mksrands(char *buf) {
  static const char charset[] =
      "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVXYZ1234567890____:;";
...
ctr_object *ctr_cwlk_assignment(ctr_tnode *node) {
  ...
  if (assignee->type == CTR_AST_NODE_REFERENCE) {
    ...
    result = ctr_send_message_variadic(
        x, "unpack:", 7, 2, y,
  ...
  case CTR_AST_NODE_ENDOFPROGRAM:
    if (CtrStdFlow && CtrStdFlow != CtrStdExit && ctr_cwlk_subprogram == 0) {
      ...
      if (CtrStdFlow->info.type == CTR_OBJECT_TYPE_OTSTRING) {
        ...
        fwrite(error->value.svalue->value, sizeof(char),
               error->value.svalue->vlen, stdout);
        printf("\n");
      }
...
ctr_object *ctr_cwlk_run(ctr_tnode *program) {
  ...
  if (wasReturn == 0 &&
      !program->lexical) // let the last value be returned on BLOCKMAP
    result = NULL;
  ctr_cwlk_msg_level--;
  return result;
}
